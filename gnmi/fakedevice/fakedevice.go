...
//func goBgpTask(q gnmit.Queue, update gnmit.UpdateFn, target string, remove func()) error {
//	peerAsPaths, _, err := ygot.ResolvePath(configpath.DeviceRoot("").NetworkInstance("default").Protocol(config.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, "BGP").Bgp().NeighborAny().PeerAs())
//	if err != nil {
//		log.Errorf("syslogTask failed to initialize due to error: %v", err)
//	}
//
