...
switch (attr_list[i].id) {
  ...
  case SAI_UDF_ATTR_BASE:
	req.set_base(static_cast<lemming::dataplane::sai::UdfBase>(attr_list[i].value.s32 + 1));
	break;
  case SAI_UDF_ATTR_OFFSET:
	req.set_offset(attr_list[i].value.u16);
	break;
  case SAI_UDF_ATTR_HASH_MASK:
	req.mutable_hash_mask()->Add(attr_list[i].value.u8list.list, attr_list[i].value.u8list.list + attr_list[i].value.u8list.count);
	break;
}
...
switch (attr->id) {
  ...
  case SAI_UDF_ATTR_BASE:
	req.set_base(static_cast<lemming::dataplane::sai::UdfBase>(attr->value.s32 + 1));
	break;
  case SAI_UDF_ATTR_HASH_MASK:
...
sai_status_t l_get_udf_attribute(sai_object_id_t udf_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::UdfAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_udf_match_attribute(sai_object_id_t udf_match_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::UdfMatchAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_UDF_GROUP_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::UdfGroupType>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_udf_group_attribute(sai_object_id_t udf_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::UdfGroupAttr>(attr_list[i].id + 1));
	}
