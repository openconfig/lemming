  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_UDF_MATCH_ATTR_L4_DST_PORT_TYPE:
        *msg.mutable_l4_dst_port_type() = convert_from_acl_field_data(
            attr_list[i].value.aclfield, attr_list[i].value.aclfield.data.u16,
            attr_list[i].value.aclfield.mask.u16);
        break;
    }
...
sai_status_t l_get_udf_match_attribute(sai_object_id_t udf_match_id,
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_UDF_MATCH_ATTR_L4_DST_PORT_TYPE:
        attr_list[i].value.aclfield = convert_to_acl_field_data_u16(
            resp.attr().l4_dst_port_type(),
            resp.attr().l4_dst_port_type().data_uint(),
            resp.attr().l4_dst_port_type().mask_uint());
        break;
    }
  }
