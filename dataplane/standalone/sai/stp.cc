...
sai_status_t l_get_stp_attribute(sai_object_id_t stp_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::StpAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_STP_PORT_ATTR_STATE:
	req.set_state(static_cast<lemming::dataplane::sai::StpPortState>(attr_list[i].value.s32 + 1));
	break;
}
...
switch (attr->id) {
  ...
  case SAI_STP_PORT_ATTR_STATE:
	req.set_state(static_cast<lemming::dataplane::sai::StpPortState>(attr->value.s32 + 1));
	break;
}
...
sai_status_t l_get_stp_port_attribute(sai_object_id_t stp_port_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::StpPortAttr>(attr_list[i].id + 1));
	}
