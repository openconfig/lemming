...
#include "dataplane/standalone/proto/my_mac.pb.h"
...
switch (attr_list[i].id) {
  ...
  case SAI_MY_MAC_ATTR_MAC_ADDRESS:
	req.set_mac_address(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
  case SAI_MY_MAC_ATTR_MAC_ADDRESS_MASK:
	req.set_mac_address_mask(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
}
...
sai_status_t l_get_my_mac_attribute(sai_object_id_t my_mac_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::MyMacAttr>(attr_list[i].id + 1));
	}
