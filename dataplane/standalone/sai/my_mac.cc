...
lemming::dataplane::sai::CreateMyMacRequest convert_create_my_mac(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_MY_MAC_ATTR_MAC_ADDRESS:
	msg.set_mac_address(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
  case SAI_MY_MAC_ATTR_MAC_ADDRESS_MASK:
	msg.set_mac_address_mask(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
}
...
sai_status_t l_create_my_mac(sai_object_id_t *my_mac_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMyMacRequest req = convert_create_my_mac(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_MY_MAC_ATTR_VLAN_ID:
	...
	break;
  case SAI_MY_MAC_ATTR_MAC_ADDRESS:
	memcpy(attr_list[i].value.mac, resp.attr().mac_address().data(), sizeof(sai_mac_t));
	break;
  case SAI_MY_MAC_ATTR_MAC_ADDRESS_MASK:
	memcpy(attr_list[i].value.mac, resp.attr().mac_address_mask().data(), sizeof(sai_mac_t));
	break;
}
