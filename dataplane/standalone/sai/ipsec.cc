...
sai_status_t l_get_ipsec_attribute(sai_object_id_t ipsec_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::IpsecAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_ipsec_port_attribute(sai_object_id_t ipsec_port_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::IpsecPortAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_DIRECTION:
	req.set_ipsec_direction(static_cast<lemming::dataplane::sai::IpsecDirection>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_PORT_LIST:
	req.mutable_ipsec_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_CIPHER:
	req.set_ipsec_cipher(static_cast<lemming::dataplane::sai::IpsecCipher>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_ipsec_sa_attribute(sai_object_id_t ipsec_sa_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::IpsecSaAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_DIRECTION:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().ipsec_direction() - 1);
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_ID:
	 attr_list[i].value.oid =   resp.attr().ipsec_id();
	break;
  case SAI_IPSEC_SA_ATTR_OCTET_COUNT_STATUS:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().octet_count_status() - 1);
	break;
  case SAI_IPSEC_SA_ATTR_EXTERNAL_SA_INDEX:
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_ESN_ENABLE:
	...
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_CIPHER:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().ipsec_cipher() - 1);
	break;
  case SAI_IPSEC_SA_ATTR_SALT:
