...
lemming::dataplane::sai::CreateIpsecRequest convert_create_ipsec(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateIpsecPortRequest convert_create_ipsec_port(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateIpsecSaRequest convert_create_ipsec_sa(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_DIRECTION:
	msg.set_ipsec_direction(convert_sai_ipsec_direction_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_PORT_LIST:
	msg.mutable_ipsec_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_CIPHER:
	msg.set_ipsec_cipher(convert_sai_ipsec_cipher_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_ipsec(sai_object_id_t *ipsec_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIpsecRequest req = convert_create_ipsec(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_IPSEC_ATTR_TERM_REMOTE_IP_MATCH_SUPPORTED:
	 attr_list[i].value.booldata =   resp.attr().term_remote_ip_match_supported();
	break;
  ...
  case SAI_IPSEC_ATTR_SUPPORTED_CIPHER_LIST:
	convert_list_sai_ipsec_cipher_t_to_sai(attr_list[i].value.s32list.list, resp.attr().supported_cipher_list(), &attr_list[i].value.s32list.count);
	break;
...
sai_status_t l_create_ipsec_port(sai_object_id_t *ipsec_port_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIpsecPortRequest req = convert_create_ipsec_port(switch_id, attr_count, attr_list);
...
sai_status_t l_create_ipsec_sa(sai_object_id_t *ipsec_sa_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIpsecSaRequest req = convert_create_ipsec_sa(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_IPSEC_SA_ATTR_EXTERNAL_SA_INDEX:
	...
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_PORT_LIST:
	req.mutable_ipsec_port_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_REPLAY_PROTECTION_ENABLE:
...
switch (attr_list[i].id) {
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_DIRECTION:
	 attr_list[i].value.s32 =  convert_sai_ipsec_direction_t_to_sai(resp.attr().ipsec_direction());
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_ID:
	 attr_list[i].value.oid =   resp.attr().ipsec_id();
	break;
  case SAI_IPSEC_SA_ATTR_OCTET_COUNT_STATUS:
	 attr_list[i].value.s32 =  convert_sai_ipsec_sa_octet_count_status_t_to_sai(resp.attr().octet_count_status());
	break;
  case SAI_IPSEC_SA_ATTR_EXTERNAL_SA_INDEX:
  ...
  case SAI_IPSEC_SA_ATTR_SA_INDEX:
	...
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_PORT_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().ipsec_port_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_SPI:
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_ESN_ENABLE:
	...
	break;
  case SAI_IPSEC_SA_ATTR_IPSEC_CIPHER:
	 attr_list[i].value.s32 =  convert_sai_ipsec_cipher_t_to_sai(resp.attr().ipsec_cipher());
	break;
  case SAI_IPSEC_SA_ATTR_SALT:
  ...
  case SAI_IPSEC_SA_ATTR_IPSEC_REPLAY_PROTECTION_WINDOW:
	...
	break;
  case SAI_IPSEC_SA_ATTR_TERM_DST_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().term_dst_ip());
	break;
  case SAI_IPSEC_SA_ATTR_TERM_VLAN_ID_ENABLE:
  ...
  case SAI_IPSEC_SA_ATTR_TERM_SRC_IP_ENABLE:
	...
	break;
  case SAI_IPSEC_SA_ATTR_TERM_SRC_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().term_src_ip());
	break;
  case SAI_IPSEC_SA_ATTR_EGRESS_ESN:
