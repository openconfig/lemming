...
lemming::dataplane::sai::CreateIcmpEchoSessionRequest convert_create_icmp_echo_session(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_icmp_echo_session(sai_object_id_t *icmp_echo_session_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	
	lemming::dataplane::sai::CreateIcmpEchoSessionRequest req = convert_create_icmp_echo_session(switch_id, attr_count, attr_list);
...
sai_status_t l_set_icmp_echo_session_attribute(sai_object_id_t icmp_echo_session_id, const sai_attribute_t *attr) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
switch (attr->id) {
  ...
  case SAI_ICMP_ECHO_SESSION_ATTR_SELECTIVE_COUNTER_LIST:
	...
	grpc::Status status = icmp_echo->SetIcmpEchoSessionAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_icmp_echo_session_attribute(sai_object_id_t icmp_echo_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_icmp_echo_session_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = icmp_echo->GetIcmpEchoSessionAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_ICMP_ECHO_SESSION_ATTR_RX_INTERVAL:
	...
	break;
  case SAI_ICMP_ECHO_SESSION_ATTR_SET_NEXT_HOP_GROUP_SWITCHOVER:
	 attr_list[i].value.booldata =   resp.attr().set_next_hop_group_switchover();
	break;
  case SAI_ICMP_ECHO_SESSION_ATTR_STATE:
	 attr_list[i].value.s32 =  convert_sai_icmp_echo_session_state_t_to_sai(resp.attr().state());
	break;
  case SAI_ICMP_ECHO_SESSION_ATTR_STATS_COUNT_MODE:
...
sai_status_t l_get_icmp_echo_session_stats(sai_object_id_t icmp_echo_session_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_icmp_echo_session_stat_t_to_proto(counter_ids[i]));
	}
	grpc::Status status = icmp_echo->GetIcmpEchoSessionStats(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_clear_icmp_echo_session_stats(sai_object_id_t icmp_echo_session_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
