...
lemming::dataplane::sai::CreateSystemPortRequest convert_create_system_port(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_system_port(sai_object_id_t *system_port_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateSystemPortRequest req = convert_create_system_port(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_SYSTEM_PORT_ATTR_QOS_TC_TO_QUEUE_MAP:
	...
	grpc::Status status = system_port->SetSystemPortAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_system_port_attribute(sai_object_id_t system_port_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	grpc::Status status = system_port->GetSystemPortAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_SYSTEM_PORT_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_system_port_type_t_to_sai(resp.attr().type());
	break;
  ...
  case SAI_SYSTEM_PORT_ATTR_QOS_VOQ_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().qos_voq_list(), &attr_list[i].value.objlist.count);
	break;
