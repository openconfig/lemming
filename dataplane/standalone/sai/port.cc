  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_PORT_ATTR_PATH_TRACING_TIMESTAMP_TYPE:
        msg.set_path_tracing_timestamp_type(
            convert_sai_port_path_tracing_timestamp_type_t_to_proto(
                attr_list[i].value.s32));
        break;
      case SAI_PORT_ATTR_DATAPATH_ENABLE:
        msg.set_datapath_enable(convert_sai_port_datapath_enable_t_to_proto(
            attr_list[i].value.s32));
        break;
      case SAI_PORT_ATTR_CABLE_TYPE:
        msg.set_cable_type(
            convert_sai_port_cable_type_t_to_proto(attr_list[i].value.s32));
        break;
...
sai_status_t l_set_port_attribute(sai_object_id_t port_id,
  ...
  switch (attr->id) {
    ...
    case SAI_PORT_ATTR_PATH_TRACING_INTF:
      ...
      break;
    case SAI_PORT_ATTR_PATH_TRACING_TIMESTAMP_TYPE:
      req.set_path_tracing_timestamp_type(
          convert_sai_port_path_tracing_timestamp_type_t_to_proto(
              attr->value.s32));
      break;
    case SAI_PORT_ATTR_DATAPATH_ENABLE:
      req.set_datapath_enable(
          convert_sai_port_datapath_enable_t_to_proto(attr->value.s32));
      break;
    case SAI_PORT_ATTR_CABLE_TYPE:
      req.set_cable_type(
          convert_sai_port_cable_type_t_to_proto(attr->value.s32));
      break;
    case SAI_PORT_ATTR_ARS_PORT_LOAD_PAST_WEIGHT:
    ...
    switch (attr_list[i].id) {
      ...
      case SAI_PORT_ATTR_JSON_FORMATTED_DEBUG_DATA:
        copy_list(attr_list[i].value.s8list.list,
                  resp.attr().json_formatted_debug_data(),
      ...
      case SAI_PORT_ATTR_OPER_PORT_FEC_MODE:
        attr_list[i].value.s32 = convert_sai_port_fec_mode_t_to_sai(
            resp.attr().oper_port_fec_mode());
        break;
      case SAI_PORT_ATTR_HOST_TX_SIGNAL_ENABLE:
        attr_list[i].value.booldata = resp.attr().host_tx_signal_enable();
        break;
      case SAI_PORT_ATTR_HOST_TX_READY_STATUS:
        attr_list[i].value.s32 = convert_sai_port_host_tx_ready_status_t_to_sai(
            resp.attr().host_tx_ready_status());
        break;
      case SAI_PORT_ATTR_PATH_TRACING_INTF:
      ...
      case SAI_PORT_ATTR_PATH_TRACING_TIMESTAMP_TYPE:
        attr_list[i].value.s32 =
            convert_sai_port_path_tracing_timestamp_type_t_to_sai(
                resp.attr().path_tracing_timestamp_type());
        break;
      case SAI_PORT_ATTR_DATAPATH_ENABLE:
        attr_list[i].value.s32 = convert_sai_port_datapath_enable_t_to_sai(
            resp.attr().datapath_enable());
        break;
      case SAI_PORT_ATTR_CABLE_PAIR_STATE:
        convert_list_sai_port_cable_pair_state_t_to_sai(
            attr_list[i].value.s32list.list, resp.attr().cable_pair_state(),
            &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_ATTR_CABLE_PAIR_LENGTH:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().cable_pair_length(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_ATTR_CABLE_TYPE:
        attr_list[i].value.s32 =
            convert_sai_port_cable_type_t_to_sai(resp.attr().cable_type());
        break;
      case SAI_PORT_ATTR_ARS_PORT_LOAD_PAST_WEIGHT:
      ...
      case SAI_PORT_ATTR_ERROR_STATUS:
        attr_list[i].value.s32 =
            convert_sai_port_error_status_t_to_sai(resp.attr().error_status());
        break;
      case SAI_PORT_ATTR_STATS_COUNT_MODE:
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_PORT_SERDES_ATTR_TX_FIR_ATTN:
        ...
        break;
      case SAI_PORT_SERDES_ATTR_TX_PAM4_RATIO:
        copy_list(attr_list[i].value.s32list.list, resp.attr().tx_pam4_ratio(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_OUT_COMMON_MODE:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().tx_out_common_mode(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_PMOS_COMMON_MODE:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().tx_pmos_common_mode(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_NMOS_COMMON_MODE:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().tx_nmos_common_mode(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_PMOS_VLTG_REG:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().tx_pmos_vltg_reg(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_NMOS_VLTG_REG:
        copy_list(attr_list[i].value.s32list.list,
                  resp.attr().tx_nmos_vltg_reg(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_TX_PRECODING:
        copy_list(attr_list[i].value.s32list.list, resp.attr().tx_precoding(),
                  &attr_list[i].value.s32list.count);
        break;
      case SAI_PORT_SERDES_ATTR_RX_PRECODING:
