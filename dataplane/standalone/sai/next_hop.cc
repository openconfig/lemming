...
lemming::dataplane::sai::CreateNextHopRequest convert_create_next_hop(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_ATTR_TYPE:
	msg.set_type(convert_sai_next_hop_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_NEXT_HOP_ATTR_TUNNEL_MAC:
	msg.set_tunnel_mac(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
  case SAI_NEXT_HOP_ATTR_SRV6_SIDLIST_ID:
	msg.set_srv6_sidlist_id(attr_list[i].value.oid);
	break;
  case SAI_NEXT_HOP_ATTR_LABELSTACK:
	msg.mutable_labelstack()->Add(attr_list[i].value.u32list.list, attr_list[i].value.u32list.list + attr_list[i].value.u32list.count);
	break;
  ...
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	msg.set_outseg_type(convert_sai_outseg_type_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	msg.set_outseg_ttl_mode(convert_sai_outseg_ttl_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	msg.set_outseg_exp_mode(convert_sai_outseg_exp_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_NEXT_HOP_ATTR_DISABLE_SRC_MAC_REWRITE:
	msg.set_disable_src_mac_rewrite(attr_list[i].value.booldata);
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_DST_MAC_REWRITE:
	msg.set_disable_dst_mac_rewrite(attr_list[i].value.booldata);
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_VLAN_REWRITE:
	msg.set_disable_vlan_rewrite(attr_list[i].value.booldata);
	break;
}
...
sai_status_t l_create_next_hop(sai_object_id_t *next_hop_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateNextHopRequest req = convert_create_next_hop(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	...
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	req.set_outseg_ttl_mode(convert_sai_outseg_ttl_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_VALUE:
	req.set_outseg_ttl_value(attr->value.u8);
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	req.set_outseg_exp_mode(convert_sai_outseg_exp_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_VALUE:
  ...
  case SAI_NEXT_HOP_ATTR_QOS_TC_AND_COLOR_TO_MPLS_EXP_MAP:
	...
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_SRC_MAC_REWRITE:
	req.set_disable_src_mac_rewrite(attr->value.booldata);
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_DST_MAC_REWRITE:
	req.set_disable_dst_mac_rewrite(attr->value.booldata);
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_VLAN_REWRITE:
	req.set_disable_vlan_rewrite(attr->value.booldata);
	break;
}
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_next_hop_type_t_to_sai(resp.attr().type());
	break;
  case SAI_NEXT_HOP_ATTR_IP:
  ...
  case SAI_NEXT_HOP_ATTR_TUNNEL_VNI:
	...
	break;
  case SAI_NEXT_HOP_ATTR_TUNNEL_MAC:
	memcpy(attr_list[i].value.mac, resp.attr().tunnel_mac().data(), sizeof(sai_mac_t));
	break;
  case SAI_NEXT_HOP_ATTR_SRV6_SIDLIST_ID:
	 attr_list[i].value.oid =   resp.attr().srv6_sidlist_id();
	break;
  case SAI_NEXT_HOP_ATTR_LABELSTACK:
	copy_list(attr_list[i].value.u32list.list, resp.attr().labelstack(), &attr_list[i].value.u32list.count);
	break;
  case SAI_NEXT_HOP_ATTR_COUNTER_ID:
  ...
  case SAI_NEXT_HOP_ATTR_DISABLE_DECREMENT_TTL:
	...
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	 attr_list[i].value.s32 =  convert_sai_outseg_type_t_to_sai(resp.attr().outseg_type());
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	 attr_list[i].value.s32 =  convert_sai_outseg_ttl_mode_t_to_sai(resp.attr().outseg_ttl_mode());
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_VALUE:
	 attr_list[i].value.u8 =   resp.attr().outseg_ttl_value();
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	 attr_list[i].value.s32 =  convert_sai_outseg_exp_mode_t_to_sai(resp.attr().outseg_exp_mode());
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_VALUE:
  ...
  case SAI_NEXT_HOP_ATTR_QOS_TC_AND_COLOR_TO_MPLS_EXP_MAP:
	...
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_SRC_MAC_REWRITE:
	 attr_list[i].value.booldata =   resp.attr().disable_src_mac_rewrite();
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_DST_MAC_REWRITE:
	 attr_list[i].value.booldata =   resp.attr().disable_dst_mac_rewrite();
	break;
  case SAI_NEXT_HOP_ATTR_DISABLE_VLAN_REWRITE:
	 attr_list[i].value.booldata =   resp.attr().disable_vlan_rewrite();
	break;
}
...
sai_status_t l_create_next_hops(sai_object_id_t switch_id, uint32_t object_count, const uint32_t *attr_count, const sai_attribute_t **attr_list, sai_bulk_op_error_mode_t mode, sai_object_id_t *object_id, sai_status_t *object_statuses) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
sai_status_t l_remove_next_hops(uint32_t object_count, const sai_object_id_t *object_id, sai_bulk_op_error_mode_t mode, sai_status_t *object_statuses) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
sai_status_t l_set_next_hops_attribute(uint32_t object_count, const sai_object_id_t *object_id, const sai_attribute_t *attr_list, sai_bulk_op_error_mode_t mode, sai_status_t *object_statuses) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	return SAI_STATUS_NOT_IMPLEMENTED;
}

sai_status_t l_get_next_hops_attribute(uint32_t object_count, const sai_object_id_t *object_id, const uint32_t *attr_count, sai_attribute_t **attr_list, sai_bulk_op_error_mode_t mode, sai_status_t *object_statuses) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	return SAI_STATUS_NOT_IMPLEMENTED;
}
