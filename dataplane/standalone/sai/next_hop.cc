...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::NextHopType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_NEXT_HOP_ATTR_TUNNEL_MAC:
	req.set_tunnel_mac(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
  case SAI_NEXT_HOP_ATTR_SRV6_SIDLIST_ID:
	req.set_srv6_sidlist_id(attr_list[i].value.oid);
	break;
  case SAI_NEXT_HOP_ATTR_LABELSTACK:
	req.mutable_labelstack()->Add(attr_list[i].value.u32list.list, attr_list[i].value.u32list.list + attr_list[i].value.u32list.count);
	break;
  ...
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	req.set_outseg_type(static_cast<lemming::dataplane::sai::OutsegType>(attr_list[i].value.s32 + 1));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	req.set_outseg_ttl_mode(static_cast<lemming::dataplane::sai::OutsegTtlMode>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	req.set_outseg_exp_mode(static_cast<lemming::dataplane::sai::OutsegExpMode>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_NEXT_HOP_ATTR_DISABLE_DECREMENT_TTL:
	...
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	req.set_outseg_type(static_cast<lemming::dataplane::sai::OutsegType>(attr->value.s32 + 1));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	req.set_outseg_ttl_mode(static_cast<lemming::dataplane::sai::OutsegTtlMode>(attr->value.s32 + 1));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_VALUE:
	req.set_outseg_ttl_value(attr->value.u8);
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	req.set_outseg_exp_mode(static_cast<lemming::dataplane::sai::OutsegExpMode>(attr->value.s32 + 1));
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_VALUE:
...
sai_status_t l_get_next_hop_attribute(sai_object_id_t next_hop_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NextHopAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_ATTR_DISABLE_DECREMENT_TTL:
	...
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().outseg_type() - 1);
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().outseg_ttl_mode() - 1);
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_TTL_VALUE:
	 attr_list[i].value.u8 =   resp.attr().outseg_ttl_value();
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().outseg_exp_mode() - 1);
	break;
  case SAI_NEXT_HOP_ATTR_OUTSEG_EXP_VALUE:
