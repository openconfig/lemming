...
const sai_hash_api_t l_hash = {
	...
	.set_fine_grained_hash_field_attribute = l_set_fine_grained_hash_field_attribute,
	.get_fine_grained_hash_field_attribute = l_get_fine_grained_hash_field_attribute,
};


lemming::dataplane::sai::CreateHashRequest convert_create_hash(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_HASH_ATTR_NATIVE_HASH_FIELD_LIST:
	msg.mutable_native_hash_field_list()->CopyFrom(convert_list_sai_native_hash_field_t_to_proto(attr_list[i].value.s32list));
	break;
  case SAI_HASH_ATTR_UDF_GROUP_LIST:
	msg.mutable_udf_group_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  case SAI_HASH_ATTR_FINE_GRAINED_HASH_FIELD_LIST:
	msg.mutable_fine_grained_hash_field_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
}
...
lemming::dataplane::sai::CreateFineGrainedHashFieldRequest convert_create_fine_grained_hash_field(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_NATIVE_HASH_FIELD:
	msg.set_native_hash_field(convert_sai_native_hash_field_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV4_MASK:
	msg.set_ipv4_mask(&attr_list[i].value.ip4, sizeof(attr_list[i].value.ip4));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV6_MASK:
	msg.set_ipv6_mask(attr_list[i].value.ip6, sizeof(attr_list[i].value.ip6));
	break;
...
sai_status_t l_create_hash(sai_object_id_t *hash_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHashRequest req = convert_create_hash(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_HASH_ATTR_NATIVE_HASH_FIELD_LIST:
	req.mutable_native_hash_field_list()->CopyFrom(convert_list_sai_native_hash_field_t_to_proto(attr->value.s32list));
	break;
  case SAI_HASH_ATTR_UDF_GROUP_LIST:
	req.mutable_udf_group_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_HASH_ATTR_FINE_GRAINED_HASH_FIELD_LIST:
	req.mutable_fine_grained_hash_field_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
}
...
switch (attr_list[i].id) {
  ...
  case SAI_HASH_ATTR_NATIVE_HASH_FIELD_LIST:
	convert_list_sai_native_hash_field_t_to_sai(attr_list[i].value.s32list.list, resp.attr().native_hash_field_list(), &attr_list[i].value.s32list.count);
	break;
  case SAI_HASH_ATTR_UDF_GROUP_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().udf_group_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_HASH_ATTR_FINE_GRAINED_HASH_FIELD_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().fine_grained_hash_field_list(), &attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_create_fine_grained_hash_field(sai_object_id_t *fine_grained_hash_field_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateFineGrainedHashFieldRequest req = convert_create_fine_grained_hash_field(switch_id, attr_count, attr_list);
...
sai_status_t l_get_fine_grained_hash_field_attribute(sai_object_id_t fine_grained_hash_field_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_fine_grained_hash_field_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = hash->GetFineGrainedHashFieldAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_NATIVE_HASH_FIELD:
	 attr_list[i].value.s32 =  convert_sai_native_hash_field_t_to_sai(resp.attr().native_hash_field());
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV4_MASK:
	memcpy(&attr_list[i].value.ip4, resp.attr().ipv4_mask().data(), sizeof(sai_ip4_t));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV6_MASK:
	memcpy(attr_list[i].value.ip6, resp.attr().ipv6_mask().data(), sizeof(sai_ip6_t));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_SEQUENCE_ID:
