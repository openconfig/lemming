...
switch (attr_list[i].id) {
  ...
  case SAI_HASH_ATTR_UDF_GROUP_LIST:
	req.mutable_udf_group_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  case SAI_HASH_ATTR_FINE_GRAINED_HASH_FIELD_LIST:
	req.mutable_fine_grained_hash_field_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_get_hash_attribute(sai_object_id_t hash_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HashAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_NATIVE_HASH_FIELD:
	req.set_native_hash_field(static_cast<lemming::dataplane::sai::NativeHashField>(attr_list[i].value.s32 + 1));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV4_MASK:
	req.set_ipv4_mask(&attr_list[i].value.ip4, sizeof(attr_list[i].value.ip4));
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV6_MASK:
	req.set_ipv6_mask(attr_list[i].value.ip6, sizeof(attr_list[i].value.ip6));
	break;
...
sai_status_t l_get_fine_grained_hash_field_attribute(sai_object_id_t fine_grained_hash_field_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::FineGrainedHashFieldAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_NATIVE_HASH_FIELD:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().native_hash_field() - 1);
	break;
  case SAI_FINE_GRAINED_HASH_FIELD_ATTR_IPV4_MASK:
