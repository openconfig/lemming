...
const sai_tam_api_t l_tam = {
    ...
    .set_tam_counter_subscription_attribute =
        l_set_tam_counter_subscription_attribute,
    .get_tam_counter_subscription_attribute =
        l_get_tam_counter_subscription_attribute,
};
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_TAM_REPORT_ATTR_REPORT_INTERVAL_UNIT:
        msg.set_report_interval_unit(
            convert_sai_tam_report_interval_unit_t_to_proto(
                attr_list[i].value.s32));
        break;
    }
  }
...
lemming::dataplane::sai::CreateTamCounterSubscriptionRequest
convert_create_tam_counter_subscription(sai_object_id_t switch_id,
                                        uint32_t attr_count,
                                        const sai_attribute_t* attr_list) {
...
sai_status_t l_set_tam_report_attribute(sai_object_id_t tam_report_id,
  ...
  switch (attr->id) {
    ...
    case SAI_TAM_REPORT_ATTR_TEMPLATE_REPORT_INTERVAL:
      ...
      break;
    case SAI_TAM_REPORT_ATTR_REPORT_INTERVAL_UNIT:
      req.set_report_interval_unit(
          convert_sai_tam_report_interval_unit_t_to_proto(attr->value.s32));
      break;
  }
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_TAM_REPORT_ATTR_TEMPLATE_REPORT_INTERVAL:
        ...
        break;
      case SAI_TAM_REPORT_ATTR_REPORT_INTERVAL_UNIT:
        attr_list[i].value.s32 = convert_sai_tam_report_interval_unit_t_to_sai(
            resp.attr().report_interval_unit());
        break;
    }
  }
...
sai_status_t l_set_tam_event_threshold_attribute(
  ...
  grpc::Status status =
      tam->SetTamEventThresholdAttribute(&context, req, &resp);
  if (!status.ok()) {
    auto it = context.GetServerTrailingMetadata().find("traceparent");
    if (it != context.GetServerTrailingMetadata().end()) {
      LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second
...
sai_status_t l_get_tam_tel_type_attribute(sai_object_id_t tam_tel_type_id,
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_PORT_STATS_INGRESS:
        attr_list[i].value.booldata =
            resp.attr().switch_enable_port_stats_ingress();
        break;
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_PORT_STATS_EGRESS:
        attr_list[i].value.booldata =
            resp.attr().switch_enable_port_stats_egress();
        break;
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_VIRTUAL_QUEUE_STATS:
        attr_list[i].value.booldata =
            resp.attr().switch_enable_virtual_queue_stats();
        break;
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_OUTPUT_QUEUE_STATS:
        attr_list[i].value.booldata =
            resp.attr().switch_enable_output_queue_stats();
        break;
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_MMU_STATS:
      ...
      case SAI_TAM_TEL_TYPE_ATTR_SWITCH_ENABLE_RESOURCE_UTILIZATION_STATS:
        attr_list[i].value.booldata =
            resp.attr().switch_enable_resource_utilization_stats();
        break;
      case SAI_TAM_TEL_TYPE_ATTR_FABRIC_Q:
      ...
      case SAI_TAM_TEL_TYPE_ATTR_COUNTER_SUBSCRIPTION_LIST:
        copy_list(attr_list[i].value.objlist.list,
                  resp.attr().counter_subscription_list(),
                  &attr_list[i].value.objlist.count);
        break;
    }
  }
...
sai_status_t l_create_tam_counter_subscription(
    sai_object_id_t* tam_counter_subscription_id, sai_object_id_t switch_id,
    uint32_t attr_count, const sai_attribute_t* attr_list) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;

  lemming::dataplane::sai::CreateTamCounterSubscriptionRequest req =
      convert_create_tam_counter_subscription(switch_id, attr_count, attr_list);
...
sai_status_t l_remove_tam_counter_subscription(
    sai_object_id_t tam_counter_subscription_id) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
sai_status_t l_set_tam_counter_subscription_attribute(
    sai_object_id_t tam_counter_subscription_id, const sai_attribute_t* attr) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;

  return SAI_STATUS_SUCCESS;
}

sai_status_t l_get_tam_counter_subscription_attribute(
    sai_object_id_t tam_counter_subscription_id, uint32_t attr_count,
    sai_attribute_t* attr_list) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    req.add_attr_type(
        convert_sai_tam_counter_subscription_attr_t_to_proto(attr_list[i].id));
  }
  grpc::Status status =
      tam->GetTamCounterSubscriptionAttribute(&context, req, &resp);
  if (!status.ok()) {
    auto it = context.GetServerTrailingMetadata().find("traceparent");
    if (it != context.GetServerTrailingMetadata().end()) {
      LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second
