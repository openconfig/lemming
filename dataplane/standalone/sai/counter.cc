...
switch (attr_list[i].id) {
  ...
  case SAI_COUNTER_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::CounterType>(attr_list[i].value.s32 + 1));
	break;
}
...
sai_status_t l_get_counter_attribute(sai_object_id_t counter_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::CounterAttr>(attr_list[i].id + 1));
	}
