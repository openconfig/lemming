...
lemming::dataplane::sai::CreateL2mcGroupRequest convert_create_l2mc_group(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateL2mcGroupMemberRequest convert_create_l2mc_group_member(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_ENDPOINT_IP:
	msg.set_l2mc_endpoint_ip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
}
...
sai_status_t l_create_l2mc_group(sai_object_id_t *l2mc_group_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateL2mcGroupRequest req = convert_create_l2mc_group(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_L2MC_GROUP_ATTR_L2MC_MEMBER_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().l2mc_member_list(), &attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_create_l2mc_group_member(sai_object_id_t *l2mc_group_member_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateL2mcGroupMemberRequest req = convert_create_l2mc_group_member(switch_id, attr_count, attr_list);
...
sai_status_t l_get_l2mc_group_member_attribute(sai_object_id_t l2mc_group_member_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_l2mc_group_member_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = l2mc_group->GetL2mcGroupMemberAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_OUTPUT_ID:
	...
	break;
  case SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_ENDPOINT_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().l2mc_endpoint_ip());
	break;
}
