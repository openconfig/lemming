...
sai_status_t l_get_l2mc_group_attribute(sai_object_id_t l2mc_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::L2mcGroupAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_ENDPOINT_IP:
	req.set_l2mc_endpoint_ip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
}
...
sai_status_t l_get_l2mc_group_member_attribute(sai_object_id_t l2mc_group_member_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::L2mcGroupMemberAttr>(attr_list[i].id + 1));
	}
