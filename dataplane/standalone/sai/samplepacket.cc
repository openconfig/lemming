...
lemming::dataplane::sai::CreateSamplepacketRequest convert_create_samplepacket(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_SAMPLEPACKET_ATTR_TYPE:
	msg.set_type(convert_sai_samplepacket_type_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_samplepacket(sai_object_id_t *samplepacket_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateSamplepacketRequest req = convert_create_samplepacket(switch_id, attr_count, attr_list);
...
sai_status_t l_get_samplepacket_attribute(sai_object_id_t samplepacket_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_samplepacket_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = samplepacket->GetSamplepacketAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_SAMPLEPACKET_ATTR_SAMPLE_RATE:
	...
	break;
  case SAI_SAMPLEPACKET_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_samplepacket_type_t_to_sai(resp.attr().type());
	break;
  case SAI_SAMPLEPACKET_ATTR_MODE:
