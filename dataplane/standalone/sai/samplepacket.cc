...
switch (attr_list[i].id) {
  ...
  case SAI_SAMPLEPACKET_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::SamplepacketType>(attr_list[i].value.s32 + 1));
	break;
  case SAI_SAMPLEPACKET_ATTR_MODE:
	req.set_mode(static_cast<lemming::dataplane::sai::SamplepacketMode>(attr_list[i].value.s32 + 1));
	break;
}
...
sai_status_t l_get_samplepacket_attribute(sai_object_id_t samplepacket_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::SamplepacketAttr>(attr_list[i].id + 1));
	}
