...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_ACTION:
	req.set_action(static_cast<lemming::dataplane::sai::TableBitmapClassificationEntryAction>(attr_list[i].value.s32 + 1));
	break;
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_ROUTER_INTERFACE_KEY:
	req.set_router_interface_key(attr_list[i].value.oid);
	break;
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_IS_DEFAULT:
	req.set_is_default(attr_list[i].value.booldata);
	break;
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_IN_RIF_METADATA:
	req.set_in_rif_metadata(attr_list[i].value.u32);
	break;
}
...
sai_status_t l_get_table_bitmap_classification_entry_attribute(sai_object_id_t table_bitmap_classification_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::TableBitmapClassificationEntryAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_ACTION:
	req.set_action(static_cast<lemming::dataplane::sai::TableBitmapRouterEntryAction>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_IN_RIF_METADATA_KEY:
	req.set_in_rif_metadata_key(attr_list[i].value.u32);
	break;
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_IN_RIF_METADATA_MASK:
	req.set_in_rif_metadata_mask(attr_list[i].value.u32);
	break;
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_TUNNEL_INDEX:
	req.set_tunnel_index(attr_list[i].value.u16);
	break;
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_NEXT_HOP:
	req.set_next_hop(attr_list[i].value.oid);
	break;
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_ROUTER_INTERFACE:
	req.set_router_interface(attr_list[i].value.oid);
	break;
...
sai_status_t l_get_table_bitmap_router_entry_attribute(sai_object_id_t table_bitmap_router_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::TableBitmapRouterEntryAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_ACTION:
	req.set_action(static_cast<lemming::dataplane::sai::TableMetaTunnelEntryAction>(attr_list[i].value.s32 + 1));
	break;
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_METADATA_KEY:
	req.set_metadata_key(attr_list[i].value.u16);
	break;
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_UNDERLAY_DIP:
	req.set_underlay_dip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
}
...
sai_status_t l_get_table_meta_tunnel_entry_attribute(sai_object_id_t table_meta_tunnel_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::TableMetaTunnelEntryAttr>(attr_list[i].id + 1));
	}
