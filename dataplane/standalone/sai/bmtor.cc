...
const sai_bmtor_api_t l_bmtor = {
	.create_table_bitmap_classification_entry = l_create_table_bitmap_classification_entry,
	.remove_table_bitmap_classification_entry = l_remove_table_bitmap_classification_entry,
	.set_table_bitmap_classification_entry_attribute = l_set_table_bitmap_classification_entry_attribute,
	.get_table_bitmap_classification_entry_attribute = l_get_table_bitmap_classification_entry_attribute,
	.get_table_bitmap_classification_entry_stats = l_get_table_bitmap_classification_entry_stats,
	.get_table_bitmap_classification_entry_stats_ext = l_get_table_bitmap_classification_entry_stats_ext,
	...
	.set_table_bitmap_router_entry_attribute = l_set_table_bitmap_router_entry_attribute,
	.get_table_bitmap_router_entry_attribute = l_get_table_bitmap_router_entry_attribute,
	.get_table_bitmap_router_entry_stats = l_get_table_bitmap_router_entry_stats,
	.get_table_bitmap_router_entry_stats_ext = l_get_table_bitmap_router_entry_stats_ext,
	.clear_table_bitmap_router_entry_stats = l_clear_table_bitmap_router_entry_stats,
	...
	.set_table_meta_tunnel_entry_attribute = l_set_table_meta_tunnel_entry_attribute,
	.get_table_meta_tunnel_entry_attribute = l_get_table_meta_tunnel_entry_attribute,
	.get_table_meta_tunnel_entry_stats = l_get_table_meta_tunnel_entry_stats,
	.get_table_meta_tunnel_entry_stats_ext = l_get_table_meta_tunnel_entry_stats_ext,
	.clear_table_meta_tunnel_entry_stats = l_clear_table_meta_tunnel_entry_stats,
};


lemming::dataplane::sai::CreateTableBitmapClassificationEntryRequest convert_create_table_bitmap_classification_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_ACTION:
	msg.set_action(convert_sai_table_bitmap_classification_entry_action_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateTableBitmapRouterEntryRequest convert_create_table_bitmap_router_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_ACTION:
	msg.set_action(convert_sai_table_bitmap_router_entry_action_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateTableMetaTunnelEntryRequest convert_create_table_meta_tunnel_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_ACTION:
	msg.set_action(convert_sai_table_meta_tunnel_entry_action_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_UNDERLAY_DIP:
	msg.set_underlay_dip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
}
...
sai_status_t l_create_table_bitmap_classification_entry(sai_object_id_t *table_bitmap_classification_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTableBitmapClassificationEntryRequest req = convert_create_table_bitmap_classification_entry(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateTableBitmapClassificationEntryResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = bmtor->CreateTableBitmapClassificationEntry(&context, req, &resp);
	if (!status.ok()) {
...
sai_status_t l_remove_table_bitmap_classification_entry(sai_object_id_t table_bitmap_classification_entry_id) {
	...
	grpc::Status status = bmtor->RemoveTableBitmapClassificationEntry(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_table_bitmap_classification_entry_attribute(sai_object_id_t table_bitmap_classification_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::GetTableBitmapClassificationEntryAttributeRequest req;
	lemming::dataplane::sai::GetTableBitmapClassificationEntryAttributeResponse resp;
	grpc::ClientContext context;
	
	req.set_oid(table_bitmap_classification_entry_id); 

	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_table_bitmap_classification_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = bmtor->GetTableBitmapClassificationEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_ACTION:
	 attr_list[i].value.s32 =  convert_sai_table_bitmap_classification_entry_action_t_to_sai(resp.attr().action());
	break;
  case SAI_TABLE_BITMAP_CLASSIFICATION_ENTRY_ATTR_ROUTER_INTERFACE_KEY:
...
sai_status_t l_get_table_bitmap_classification_entry_stats(sai_object_id_t table_bitmap_classification_entry_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_table_bitmap_classification_entry_stat_t_to_proto(counter_ids[i]));
	}
	grpc::Status status = bmtor->GetTableBitmapClassificationEntryStats(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_create_table_bitmap_router_entry(sai_object_id_t *table_bitmap_router_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTableBitmapRouterEntryRequest req = convert_create_table_bitmap_router_entry(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateTableBitmapRouterEntryResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = bmtor->CreateTableBitmapRouterEntry(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_table_bitmap_router_entry(sai_object_id_t table_bitmap_router_entry_id) {
	...
	grpc::Status status = bmtor->RemoveTableBitmapRouterEntry(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_table_bitmap_router_entry_attribute(sai_object_id_t table_bitmap_router_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_table_bitmap_router_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = bmtor->GetTableBitmapRouterEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_ACTION:
	 attr_list[i].value.s32 =  convert_sai_table_bitmap_router_entry_action_t_to_sai(resp.attr().action());
	break;
  case SAI_TABLE_BITMAP_ROUTER_ENTRY_ATTR_PRIORITY:
...
sai_status_t l_get_table_bitmap_router_entry_stats(sai_object_id_t table_bitmap_router_entry_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_table_bitmap_router_entry_stat_t_to_proto(counter_ids[i]));
	}
	grpc::Status status = bmtor->GetTableBitmapRouterEntryStats(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_create_table_meta_tunnel_entry(sai_object_id_t *table_meta_tunnel_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTableMetaTunnelEntryRequest req = convert_create_table_meta_tunnel_entry(switch_id, attr_count, attr_list);
...
sai_status_t l_get_table_meta_tunnel_entry_attribute(sai_object_id_t table_meta_tunnel_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_table_meta_tunnel_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = bmtor->GetTableMetaTunnelEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_ACTION:
	 attr_list[i].value.s32 =  convert_sai_table_meta_tunnel_entry_action_t_to_sai(resp.attr().action());
	break;
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_METADATA_KEY:
  ...
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_TUNNEL_ID:
	...
	break;
  case SAI_TABLE_META_TUNNEL_ENTRY_ATTR_UNDERLAY_DIP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().underlay_dip());
	break;
}
...
sai_status_t l_get_table_meta_tunnel_entry_stats(sai_object_id_t table_meta_tunnel_entry_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_table_meta_tunnel_entry_stat_t_to_proto(counter_ids[i]));
	}
	grpc::Status status = bmtor->GetTableMetaTunnelEntryStats(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
