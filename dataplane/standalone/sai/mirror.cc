...
switch (attr_list[i].id) {
  ...
  case SAI_MIRROR_SESSION_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::MirrorSessionType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_MIRROR_SESSION_ATTR_CONGESTION_MODE:
	req.set_congestion_mode(static_cast<lemming::dataplane::sai::MirrorSessionCongestionMode>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_MIRROR_SESSION_ATTR_ERSPAN_ENCAPSULATION_TYPE:
	req.set_erspan_encapsulation_type(static_cast<lemming::dataplane::sai::ErspanEncapsulationType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_MIRROR_SESSION_ATTR_MONITOR_PORTLIST:
	req.mutable_monitor_portlist()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
switch (attr->id) {
  ...
  case SAI_MIRROR_SESSION_ATTR_SAMPLE_RATE:
	...
	break;
  case SAI_MIRROR_SESSION_ATTR_CONGESTION_MODE:
	req.set_congestion_mode(static_cast<lemming::dataplane::sai::MirrorSessionCongestionMode>(attr->value.s32 + 1));
	break;
  case SAI_MIRROR_SESSION_ATTR_TC:
...
sai_status_t l_get_mirror_session_attribute(sai_object_id_t mirror_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::MirrorSessionAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_MIRROR_SESSION_ATTR_SAMPLE_RATE:
	...
	break;
  case SAI_MIRROR_SESSION_ATTR_CONGESTION_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().congestion_mode() - 1);
	break;
  case SAI_MIRROR_SESSION_ATTR_TC:
  ...
  case SAI_MIRROR_SESSION_ATTR_VLAN_HEADER_VALID:
	...
	break;
  case SAI_MIRROR_SESSION_ATTR_ERSPAN_ENCAPSULATION_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().erspan_encapsulation_type() - 1);
	break;
  case SAI_MIRROR_SESSION_ATTR_IPHDR_VERSION:
