...
lemming::dataplane::sai::CreateMacsecRequest convert_create_macsec(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_ATTR_DIRECTION:
	msg.set_direction(convert_sai_macsec_direction_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_MACSEC_ATTR_STATS_MODE:
	msg.set_stats_mode(convert_sai_stats_mode_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateMacsecPortRequest convert_create_macsec_port(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateMacsecFlowRequest convert_create_macsec_flow(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateMacsecScRequest convert_create_macsec_sc(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_SC_ATTR_MACSEC_CIPHER_SUITE:
	msg.set_macsec_cipher_suite(convert_sai_macsec_cipher_suite_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateMacsecSaRequest convert_create_macsec_sa(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_macsec(sai_object_id_t *macsec_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMacsecRequest req = convert_create_macsec(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_ATTR_DIRECTION:
	 attr_list[i].value.s32 =  convert_sai_macsec_direction_t_to_sai(resp.attr().direction());
	break;
  case SAI_MACSEC_ATTR_SWITCHING_MODE_CUT_THROUGH_SUPPORTED:
	 attr_list[i].value.booldata =   resp.attr().switching_mode_cut_through_supported();
	break;
  case SAI_MACSEC_ATTR_SWITCHING_MODE_STORE_AND_FORWARD_SUPPORTED:
	 attr_list[i].value.booldata =   resp.attr().switching_mode_store_and_forward_supported();
	break;
  ...
  case SAI_MACSEC_ATTR_STATS_MODE_READ_CLEAR_SUPPORTED:
	 attr_list[i].value.booldata =   resp.attr().stats_mode_read_clear_supported();
	break;
  ...
  case SAI_MACSEC_ATTR_SUPPORTED_CIPHER_SUITE_LIST:
	convert_list_sai_macsec_cipher_suite_t_to_sai(attr_list[i].value.s32list.list, resp.attr().supported_cipher_suite_list(), &attr_list[i].value.s32list.count);
	break;
  ...
  case SAI_MACSEC_ATTR_SECTAG_OFFSETS_SUPPORTED:
	copy_list(attr_list[i].value.u8list.list, resp.attr().sectag_offsets_supported(), &attr_list[i].value.u8list.count);
	break;
  ...
  case SAI_MACSEC_ATTR_SUPPORTED_PORT_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().supported_port_list(), &attr_list[i].value.objlist.count);
	break;
  ...
  case SAI_MACSEC_ATTR_FLOW_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().flow_list(), &attr_list[i].value.objlist.count);
	break;
  ...
  case SAI_MACSEC_ATTR_MAX_SECURE_ASSOCIATIONS_PER_SC:
	 attr_list[i].value.s32 =  convert_sai_macsec_max_secure_associations_per_sc_t_to_sai(resp.attr().max_secure_associations_per_sc());
	break;
}
...
sai_status_t l_create_macsec_port(sai_object_id_t *macsec_port_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMacsecPortRequest req = convert_create_macsec_port(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_PORT_ATTR_MACSEC_DIRECTION:
	 attr_list[i].value.s32 =  convert_sai_macsec_direction_t_to_sai(resp.attr().macsec_direction());
	break;
  case SAI_MACSEC_PORT_ATTR_PORT_ID:
...
sai_status_t l_get_macsec_port_stats(sai_object_id_t macsec_port_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_macsec_port_stat_t_to_proto(counter_ids[i]));
	}
...
sai_status_t l_create_macsec_flow(sai_object_id_t *macsec_flow_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMacsecFlowRequest req = convert_create_macsec_flow(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_FLOW_ATTR_ACL_ENTRY_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().acl_entry_list(), &attr_list[i].value.objlist.count);
	break;
...
sai_status_t l_get_macsec_flow_stats(sai_object_id_t macsec_flow_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_macsec_flow_stat_t_to_proto(counter_ids[i]));
	}
...
sai_status_t l_create_macsec_sc(sai_object_id_t *macsec_sc_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMacsecScRequest req = convert_create_macsec_sc(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_MACSEC_SC_ATTR_MACSEC_REPLAY_PROTECTION_WINDOW:
	...
	break;
  case SAI_MACSEC_SC_ATTR_MACSEC_CIPHER_SUITE:
	req.set_macsec_cipher_suite(convert_sai_macsec_cipher_suite_t_to_proto(attr->value.s32));
	break;
  case SAI_MACSEC_SC_ATTR_ENCRYPTION_ENABLE:
...
switch (attr_list[i].id) {
  ...
  case SAI_MACSEC_SC_ATTR_ACTIVE_EGRESS_SA_ID:
	...
	break;
  case SAI_MACSEC_SC_ATTR_MACSEC_REPLAY_PROTECTION_ENABLE:
	 attr_list[i].value.booldata =   resp.attr().macsec_replay_protection_enable();
	break;
  case SAI_MACSEC_SC_ATTR_MACSEC_REPLAY_PROTECTION_WINDOW:
	 attr_list[i].value.u32 =   resp.attr().macsec_replay_protection_window();
	break;
  case SAI_MACSEC_SC_ATTR_SA_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().sa_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_MACSEC_SC_ATTR_MACSEC_CIPHER_SUITE:
	 attr_list[i].value.s32 =  convert_sai_macsec_cipher_suite_t_to_sai(resp.attr().macsec_cipher_suite());
	break;
  case SAI_MACSEC_SC_ATTR_ENCRYPTION_ENABLE:
...
sai_status_t l_create_macsec_sa(sai_object_id_t *macsec_sa_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateMacsecSaRequest req = convert_create_macsec_sa(switch_id, attr_count, attr_list);
