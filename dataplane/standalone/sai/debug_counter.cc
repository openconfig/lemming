...
lemming::dataplane::sai::CreateDebugCounterRequest convert_create_debug_counter(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_DEBUG_COUNTER_ATTR_TYPE:
	msg.set_type(convert_sai_debug_counter_type_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_DEBUG_COUNTER_ATTR_BIND_METHOD:
	msg.set_bind_method(convert_sai_debug_counter_bind_method_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_DEBUG_COUNTER_ATTR_IN_DROP_REASON_LIST:
	msg.mutable_in_drop_reason_list()->CopyFrom(convert_list_sai_in_drop_reason_t_to_proto(attr_list[i].value.s32list));
	break;
  case SAI_DEBUG_COUNTER_ATTR_OUT_DROP_REASON_LIST:
	msg.mutable_out_drop_reason_list()->CopyFrom(convert_list_sai_out_drop_reason_t_to_proto(attr_list[i].value.s32list));
	break;
}
...
sai_status_t l_create_debug_counter(sai_object_id_t *debug_counter_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDebugCounterRequest req = convert_create_debug_counter(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_DEBUG_COUNTER_ATTR_IN_DROP_REASON_LIST:
	req.mutable_in_drop_reason_list()->CopyFrom(convert_list_sai_in_drop_reason_t_to_proto(attr->value.s32list));
	break;
  case SAI_DEBUG_COUNTER_ATTR_OUT_DROP_REASON_LIST:
	req.mutable_out_drop_reason_list()->CopyFrom(convert_list_sai_out_drop_reason_t_to_proto(attr->value.s32list));
	break;
}

	grpc::Status status = debug_counter->SetDebugCounterAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_debug_counter_attribute(sai_object_id_t debug_counter_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_debug_counter_attr_t_to_proto(attr_list[i].id));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_DEBUG_COUNTER_ATTR_INDEX:
	...
	break;
  case SAI_DEBUG_COUNTER_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_debug_counter_type_t_to_sai(resp.attr().type());
	break;
  case SAI_DEBUG_COUNTER_ATTR_BIND_METHOD:
	 attr_list[i].value.s32 =  convert_sai_debug_counter_bind_method_t_to_sai(resp.attr().bind_method());
	break;
  case SAI_DEBUG_COUNTER_ATTR_IN_DROP_REASON_LIST:
	convert_list_sai_in_drop_reason_t_to_sai(attr_list[i].value.s32list.list, resp.attr().in_drop_reason_list(), &attr_list[i].value.s32list.count);
	break;
  case SAI_DEBUG_COUNTER_ATTR_OUT_DROP_REASON_LIST:
	convert_list_sai_out_drop_reason_t_to_sai(attr_list[i].value.s32list.list, resp.attr().out_drop_reason_list(), &attr_list[i].value.s32list.count);
	break;
}
