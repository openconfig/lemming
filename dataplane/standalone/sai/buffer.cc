...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_POOL_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::BufferPoolType>(attr_list[i].value.s32 + 1));
	break;
  case SAI_BUFFER_POOL_ATTR_SIZE:
	req.set_size(attr_list[i].value.u64);
	break;
  case SAI_BUFFER_POOL_ATTR_THRESHOLD_MODE:
	req.set_threshold_mode(static_cast<lemming::dataplane::sai::BufferPoolThresholdMode>(attr_list[i].value.s32 + 1));
	break;
  case SAI_BUFFER_POOL_ATTR_TAM:
	req.mutable_tam()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
sai_status_t l_get_buffer_pool_attribute(sai_object_id_t buffer_pool_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::BufferPoolAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_POOL_ATTR_SIZE:
	...
	break;
  case SAI_BUFFER_POOL_ATTR_THRESHOLD_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().threshold_mode() - 1);
	break;
  case SAI_BUFFER_POOL_ATTR_TAM:
...
sai_status_t l_get_ingress_priority_group_attribute(sai_object_id_t ingress_priority_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::IngressPriorityGroupAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_PROFILE_ATTR_THRESHOLD_MODE:
	req.set_threshold_mode(static_cast<lemming::dataplane::sai::BufferProfileThresholdMode>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_buffer_profile_attribute(sai_object_id_t buffer_profile_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::BufferProfileAttr>(attr_list[i].id + 1));
	}
