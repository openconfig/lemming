...
lemming::dataplane::sai::CreateBufferPoolRequest convert_create_buffer_pool(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_POOL_ATTR_TYPE:
	msg.set_type(convert_sai_buffer_pool_type_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_BUFFER_POOL_ATTR_SIZE:
	msg.set_size(attr_list[i].value.u64);
	break;
  case SAI_BUFFER_POOL_ATTR_THRESHOLD_MODE:
	msg.set_threshold_mode(convert_sai_buffer_pool_threshold_mode_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_BUFFER_POOL_ATTR_TAM:
	msg.mutable_tam()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
lemming::dataplane::sai::CreateIngressPriorityGroupRequest convert_create_ingress_priority_group(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateBufferProfileRequest convert_create_buffer_profile(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_PROFILE_ATTR_THRESHOLD_MODE:
	msg.set_threshold_mode(convert_sai_buffer_profile_threshold_mode_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_buffer_pool(sai_object_id_t *buffer_pool_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateBufferPoolRequest req = convert_create_buffer_pool(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_POOL_ATTR_SHARED_SIZE:
	...
	break;
  case SAI_BUFFER_POOL_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_buffer_pool_type_t_to_sai(resp.attr().type());
	break;
  case SAI_BUFFER_POOL_ATTR_SIZE:
	 attr_list[i].value.u64 =   resp.attr().size();
	break;
  case SAI_BUFFER_POOL_ATTR_THRESHOLD_MODE:
	 attr_list[i].value.s32 =  convert_sai_buffer_pool_threshold_mode_t_to_sai(resp.attr().threshold_mode());
	break;
  case SAI_BUFFER_POOL_ATTR_TAM:
	copy_list(attr_list[i].value.objlist.list, resp.attr().tam(), &attr_list[i].value.objlist.count);
	break;
  case SAI_BUFFER_POOL_ATTR_XOFF_SIZE:
...
sai_status_t l_get_buffer_pool_stats(sai_object_id_t buffer_pool_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_buffer_pool_stat_t_to_proto(counter_ids[i]));
	}
...
sai_status_t l_create_ingress_priority_group(sai_object_id_t *ingress_priority_group_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIngressPriorityGroupRequest req = convert_create_ingress_priority_group(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateIngressPriorityGroupResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = buffer->CreateIngressPriorityGroup(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_ingress_priority_group(sai_object_id_t ingress_priority_group_id) {
	...
	grpc::Status status = buffer->RemoveIngressPriorityGroup(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_ingress_priority_group_attribute(sai_object_id_t ingress_priority_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_ingress_priority_group_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = buffer->GetIngressPriorityGroupAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_ingress_priority_group_stats(sai_object_id_t ingress_priority_group_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_ingress_priority_group_stat_t_to_proto(counter_ids[i]));
	}
	grpc::Status status = buffer->GetIngressPriorityGroupStats(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_create_buffer_profile(sai_object_id_t *buffer_profile_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateBufferProfileRequest req = convert_create_buffer_profile(switch_id, attr_count, attr_list);
...
sai_status_t l_get_buffer_profile_attribute(sai_object_id_t buffer_profile_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_buffer_profile_attr_t_to_proto(attr_list[i].id));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_BUFFER_PROFILE_ATTR_RESERVED_BUFFER_SIZE:
	...
	break;
  case SAI_BUFFER_PROFILE_ATTR_THRESHOLD_MODE:
	 attr_list[i].value.s32 =  convert_sai_buffer_profile_threshold_mode_t_to_sai(resp.attr().threshold_mode());
	break;
  case SAI_BUFFER_PROFILE_ATTR_SHARED_DYNAMIC_TH:
