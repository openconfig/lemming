...
lemming::dataplane::sai::CreateGenericProgrammableRequest convert_create_generic_programmable(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_GENERIC_PROGRAMMABLE_ATTR_OBJECT_NAME:
	msg.mutable_object_name()->Add(attr_list[i].value.s8list.list, attr_list[i].value.s8list.list + attr_list[i].value.s8list.count);
	break;
...
sai_status_t l_create_generic_programmable(sai_object_id_t *generic_programmable_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateGenericProgrammableRequest req = convert_create_generic_programmable(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateGenericProgrammableResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = generic_programmable->CreateGenericProgrammable(&context, req, &resp);
	if (!status.ok()) {
...
sai_status_t l_remove_generic_programmable(sai_object_id_t generic_programmable_id) {
	...
	grpc::Status status = generic_programmable->RemoveGenericProgrammable(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr->id) {
  ...
  case SAI_GENERIC_PROGRAMMABLE_ATTR_COUNTER_ID:
	...
	grpc::Status status = generic_programmable->SetGenericProgrammableAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_generic_programmable_attribute(sai_object_id_t generic_programmable_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_generic_programmable_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = generic_programmable->GetGenericProgrammableAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_GENERIC_PROGRAMMABLE_ATTR_OBJECT_NAME:
	copy_list(attr_list[i].value.s8list.list, resp.attr().object_name(), &attr_list[i].value.s8list.count);
	break;
  case SAI_GENERIC_PROGRAMMABLE_ATTR_COUNTER_ID:
