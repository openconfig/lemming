...
switch (attr_list[i].id) {
  ...
  case SAI_WRED_ATTR_ECN_MARK_MODE:
	req.set_ecn_mark_mode(static_cast<lemming::dataplane::sai::EcnMarkMode>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_WRED_ATTR_WEIGHT:
	...
	break;
  case SAI_WRED_ATTR_ECN_MARK_MODE:
	req.set_ecn_mark_mode(static_cast<lemming::dataplane::sai::EcnMarkMode>(attr->value.s32 + 1));
	break;
  case SAI_WRED_ATTR_ECN_GREEN_MIN_THRESHOLD:
...
sai_status_t l_get_wred_attribute(sai_object_id_t wred_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::WredAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_WRED_ATTR_WEIGHT:
	...
	break;
  case SAI_WRED_ATTR_ECN_MARK_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().ecn_mark_mode() - 1);
	break;
  case SAI_WRED_ATTR_ECN_GREEN_MIN_THRESHOLD:
