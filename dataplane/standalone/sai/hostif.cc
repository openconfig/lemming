...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::HostifType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_HOSTIF_ATTR_VLAN_TAG:
	req.set_vlan_tag(static_cast<lemming::dataplane::sai::HostifVlanTag>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_HOSTIF_ATTR_QUEUE:
	...
	break;
  case SAI_HOSTIF_ATTR_VLAN_TAG:
	req.set_vlan_tag(static_cast<lemming::dataplane::sai::HostifVlanTag>(attr->value.s32 + 1));
	break;
}
...
sai_status_t l_get_hostif_attribute(sai_object_id_t hostif_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HostifAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::HostifTableEntryType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_CHANNEL_TYPE:
	req.set_channel_type(static_cast<lemming::dataplane::sai::HostifTableEntryChannelType>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_hostif_table_entry_attribute(sai_object_id_t hostif_table_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HostifTableEntryAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_TRAP_ID:
	...
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_CHANNEL_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().channel_type() - 1);
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_HOST_IF:
...
sai_status_t l_get_hostif_trap_group_attribute(sai_object_id_t hostif_trap_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HostifTrapGroupAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TRAP_ATTR_TRAP_TYPE:
	req.set_trap_type(static_cast<lemming::dataplane::sai::HostifTrapType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_HOSTIF_TRAP_ATTR_EXCLUDE_PORT_LIST:
	req.mutable_exclude_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_TRAP_GROUP:
	req.set_trap_group(attr_list[i].value.oid);
	break;
  case SAI_HOSTIF_TRAP_ATTR_MIRROR_SESSION:
	req.mutable_mirror_session()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
sai_status_t l_get_hostif_trap_attribute(sai_object_id_t hostif_trap_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HostifTrapAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_USER_DEFINED_TRAP_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::HostifUserDefinedTrapType>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_hostif_user_defined_trap_attribute(sai_object_id_t hostif_user_defined_trap_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::HostifUserDefinedTrapAttr>(attr_list[i].id + 1));
	}
