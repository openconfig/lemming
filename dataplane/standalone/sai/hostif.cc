...
lemming::dataplane::sai::CreateHostifRequest convert_create_hostif(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_ATTR_TYPE:
	msg.set_type(convert_sai_hostif_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_HOSTIF_ATTR_VLAN_TAG:
	msg.set_vlan_tag(convert_sai_hostif_vlan_tag_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateHostifTableEntryRequest convert_create_hostif_table_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_TYPE:
	msg.set_type(convert_sai_hostif_table_entry_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_CHANNEL_TYPE:
	msg.set_channel_type(convert_sai_hostif_table_entry_channel_type_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateHostifTrapGroupRequest convert_create_hostif_trap_group(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateHostifTrapRequest convert_create_hostif_trap(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TRAP_ATTR_TRAP_TYPE:
	msg.set_trap_type(convert_sai_hostif_trap_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_HOSTIF_TRAP_ATTR_EXCLUDE_PORT_LIST:
	msg.mutable_exclude_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_TRAP_GROUP:
	msg.set_trap_group(attr_list[i].value.oid);
	break;
  case SAI_HOSTIF_TRAP_ATTR_MIRROR_SESSION:
	msg.mutable_mirror_session()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
lemming::dataplane::sai::CreateHostifUserDefinedTrapRequest convert_create_hostif_user_defined_trap(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_USER_DEFINED_TRAP_ATTR_TYPE:
	msg.set_type(convert_sai_hostif_user_defined_trap_type_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_hostif(sai_object_id_t *hostif_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHostifRequest req = convert_create_hostif(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_hostif_type_t_to_sai(resp.attr().type());
	break;
  case SAI_HOSTIF_ATTR_OBJ_ID:
  ...
  case SAI_HOSTIF_ATTR_QUEUE:
	...
	break;
  case SAI_HOSTIF_ATTR_VLAN_TAG:
	 attr_list[i].value.s32 =  convert_sai_hostif_vlan_tag_t_to_sai(resp.attr().vlan_tag());
	break;
  case SAI_HOSTIF_ATTR_GENETLINK_MCGRP_NAME:
	strncpy(attr_list[i].value.chardata, resp.attr().genetlink_mcgrp_name().data(), 32);
	break;
}
...
sai_status_t l_create_hostif_table_entry(sai_object_id_t *hostif_table_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHostifTableEntryRequest req = convert_create_hostif_table_entry(switch_id, attr_count, attr_list);
...
sai_status_t l_get_hostif_table_entry_attribute(sai_object_id_t hostif_table_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_hostif_table_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = hostif->GetHostifTableEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_hostif_table_entry_type_t_to_sai(resp.attr().type());
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_OBJ_ID:
	 attr_list[i].value.oid =   resp.attr().obj_id();
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_TRAP_ID:
	...
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_CHANNEL_TYPE:
	 attr_list[i].value.s32 =  convert_sai_hostif_table_entry_channel_type_t_to_sai(resp.attr().channel_type());
	break;
  case SAI_HOSTIF_TABLE_ENTRY_ATTR_HOST_IF:
...
sai_status_t l_create_hostif_trap_group(sai_object_id_t *hostif_trap_group_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHostifTrapGroupRequest req = convert_create_hostif_trap_group(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_HOSTIF_TRAP_GROUP_ATTR_POLICER:
	...
	grpc::Status status = hostif->SetHostifTrapGroupAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_hostif_trap_group_attribute(sai_object_id_t hostif_trap_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_hostif_trap_group_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = hostif->GetHostifTrapGroupAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_create_hostif_trap(sai_object_id_t *hostif_trap_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHostifTrapRequest req = convert_create_hostif_trap(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_HOSTIF_TRAP_ATTR_TRAP_PRIORITY:
	...
	break;
  case SAI_HOSTIF_TRAP_ATTR_EXCLUDE_PORT_LIST:
	req.mutable_exclude_port_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_TRAP_GROUP:
	req.set_trap_group(attr->value.oid);
	break;
  case SAI_HOSTIF_TRAP_ATTR_MIRROR_SESSION:
	req.mutable_mirror_session()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_COUNTER_ID:
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_TRAP_ATTR_TRAP_TYPE:
	 attr_list[i].value.s32 =  convert_sai_hostif_trap_type_t_to_sai(resp.attr().trap_type());
	break;
  case SAI_HOSTIF_TRAP_ATTR_PACKET_ACTION:
  ...
  case SAI_HOSTIF_TRAP_ATTR_TRAP_PRIORITY:
	...
	break;
  case SAI_HOSTIF_TRAP_ATTR_EXCLUDE_PORT_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().exclude_port_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_TRAP_GROUP:
	 attr_list[i].value.oid =   resp.attr().trap_group();
	break;
  case SAI_HOSTIF_TRAP_ATTR_MIRROR_SESSION:
	copy_list(attr_list[i].value.objlist.list, resp.attr().mirror_session(), &attr_list[i].value.objlist.count);
	break;
  case SAI_HOSTIF_TRAP_ATTR_COUNTER_ID:
...
sai_status_t l_create_hostif_user_defined_trap(sai_object_id_t *hostif_user_defined_trap_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateHostifUserDefinedTrapRequest req = convert_create_hostif_user_defined_trap(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateHostifUserDefinedTrapResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = hostif->CreateHostifUserDefinedTrap(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_hostif_user_defined_trap(sai_object_id_t hostif_user_defined_trap_id) {
	...
	grpc::Status status = hostif->RemoveHostifUserDefinedTrap(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr->id) {
  ...
  case SAI_HOSTIF_USER_DEFINED_TRAP_ATTR_TRAP_GROUP:
	...
	grpc::Status status = hostif->SetHostifUserDefinedTrapAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_hostif_user_defined_trap_attribute(sai_object_id_t hostif_user_defined_trap_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_hostif_user_defined_trap_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = hostif->GetHostifUserDefinedTrapAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_HOSTIF_USER_DEFINED_TRAP_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_hostif_user_defined_trap_type_t_to_sai(resp.attr().type());
	break;
  case SAI_HOSTIF_USER_DEFINED_TRAP_ATTR_TRAP_PRIORITY:
	 attr_list[i].value.u32 =   resp.attr().trap_priority();
	break;
