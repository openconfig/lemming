...
switch (attr_list[i].id) {
  ...
  case SAI_NAT_ENTRY_ATTR_NAT_TYPE:
	req.set_nat_type(static_cast<lemming::dataplane::sai::NatType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_NAT_ENTRY_ATTR_SRC_IP_MASK:
	req.set_src_ip_mask(&attr_list[i].value.ip4, sizeof(attr_list[i].value.ip4));
	break;
  ...
  case SAI_NAT_ENTRY_ATTR_DST_IP_MASK:
	req.set_dst_ip_mask(&attr_list[i].value.ip4, sizeof(attr_list[i].value.ip4));
	break;
...
switch (attr->id) {
  ...
  case SAI_NAT_ENTRY_ATTR_NAT_TYPE:
	req.set_nat_type(static_cast<lemming::dataplane::sai::NatType>(attr->value.s32 + 1));
	break;
  case SAI_NAT_ENTRY_ATTR_SRC_IP:
...
sai_status_t l_get_nat_entry_attribute(const sai_nat_entry_t *nat_entry, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NatEntryAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_nat_zone_counter_attribute(sai_object_id_t nat_zone_counter_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NatZoneCounterAttr>(attr_list[i].id + 1));
	}
