...
lemming::dataplane::sai::CreateRpfGroupRequest convert_create_rpf_group(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateRpfGroupMemberRequest convert_create_rpf_group_member(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_rpf_group(sai_object_id_t *rpf_group_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateRpfGroupRequest req = convert_create_rpf_group(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_RPF_GROUP_ATTR_RPF_MEMBER_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().rpf_member_list(), &attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_create_rpf_group_member(sai_object_id_t *rpf_group_member_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateRpfGroupMemberRequest req = convert_create_rpf_group_member(switch_id, attr_count, attr_list);
...
sai_status_t l_get_rpf_group_member_attribute(sai_object_id_t rpf_group_member_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_rpf_group_member_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = rpf_group->GetRpfGroupMemberAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
