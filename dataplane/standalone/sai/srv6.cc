  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_SRV6_SIDLIST_ATTR_NEXT_HOP_ID:
        ...
        break;
      case SAI_SRV6_SIDLIST_ATTR_STATS_COUNT_MODE:
        msg.set_stats_count_mode(
            convert_sai_stats_count_mode_t_to_proto(attr_list[i].value.s32));
        break;
      case SAI_SRV6_SIDLIST_ATTR_SELECTIVE_COUNTER_LIST:
...
sai_status_t l_get_srv6_sidlist_stats(sai_object_id_t srv6_sidlist_id,
  ...
  for (uint32_t i = 0; i < number_of_counters; i++) {
    req.add_counter_ids(
        convert_sai_srv6_sidlist_stat_t_to_proto(counter_ids[i]));
  }
...
sai_status_t l_get_srv6_sidlist_stats_ext(sai_object_id_t srv6_sidlist_id,
                                          uint32_t number_of_counters,
                                          const sai_stat_id_t* counter_ids,
...
sai_status_t l_clear_srv6_sidlist_stats(sai_object_id_t srv6_sidlist_id,
                                        uint32_t number_of_counters,
                                        const sai_stat_id_t* counter_ids) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
