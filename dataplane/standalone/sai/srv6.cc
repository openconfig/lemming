...
switch (attr_list[i].id) {
  ...
  case SAI_SRV6_SIDLIST_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::Srv6SidlistType>(attr_list[i].value.s32 + 1));
	break;
}
...
sai_status_t l_get_srv6_sidlist_attribute(sai_object_id_t srv6_sidlist_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::Srv6SidlistAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR:
	req.set_endpoint_behavior(static_cast<lemming::dataplane::sai::MySidEntryEndpointBehavior>(attr_list[i].value.s32 + 1));
	break;
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR_FLAVOR:
	req.set_endpoint_behavior_flavor(static_cast<lemming::dataplane::sai::MySidEntryEndpointBehaviorFlavor>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR:
	req.set_endpoint_behavior(static_cast<lemming::dataplane::sai::MySidEntryEndpointBehavior>(attr->value.s32 + 1));
	break;
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR_FLAVOR:
	req.set_endpoint_behavior_flavor(static_cast<lemming::dataplane::sai::MySidEntryEndpointBehaviorFlavor>(attr->value.s32 + 1));
	break;
  case SAI_MY_SID_ENTRY_ATTR_PACKET_ACTION:
...
sai_status_t l_get_my_sid_entry_attribute(const sai_my_sid_entry_t *my_sid_entry, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::MySidEntryAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().endpoint_behavior() - 1);
	break;
  case SAI_MY_SID_ENTRY_ATTR_ENDPOINT_BEHAVIOR_FLAVOR:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().endpoint_behavior_flavor() - 1);
	break;
  case SAI_MY_SID_ENTRY_ATTR_PACKET_ACTION:
