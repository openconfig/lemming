...
switch (attr_list[i].id) {
  ...
  case SAI_BFD_SESSION_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::BfdSessionType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_BFD_ENCAPSULATION_TYPE:
	req.set_bfd_encapsulation_type(static_cast<lemming::dataplane::sai::BfdEncapsulationType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_SRC_IP_ADDRESS:
	req.set_src_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  case SAI_BFD_SESSION_ATTR_DST_IP_ADDRESS:
	req.set_dst_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_TUNNEL_SRC_IP_ADDRESS:
	req.set_tunnel_src_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  case SAI_BFD_SESSION_ATTR_TUNNEL_DST_IP_ADDRESS:
	req.set_tunnel_dst_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_DST_MAC_ADDRESS:
	req.set_dst_mac_address(attr_list[i].value.mac, sizeof(attr_list[i].value.mac));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_OFFLOAD_TYPE:
	req.set_offload_type(static_cast<lemming::dataplane::sai::BfdSessionOffloadType>(attr_list[i].value.s32 + 1));
	break;
}
...
sai_status_t l_get_bfd_session_attribute(sai_object_id_t bfd_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::BfdSessionAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_BFD_SESSION_ATTR_VLAN_HEADER_VALID:
	...
	break;
  case SAI_BFD_SESSION_ATTR_BFD_ENCAPSULATION_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().bfd_encapsulation_type() - 1);
	break;
  case SAI_BFD_SESSION_ATTR_IPHDR_VERSION:
  ...
  case SAI_BFD_SESSION_ATTR_STATE:
	...
	break;
  case SAI_BFD_SESSION_ATTR_OFFLOAD_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().offload_type() - 1);
	break;
