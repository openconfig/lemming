...
lemming::dataplane::sai::CreateBfdSessionRequest convert_create_bfd_session(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_BFD_SESSION_ATTR_TYPE:
	msg.set_type(convert_sai_bfd_session_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_BFD_ENCAPSULATION_TYPE:
	msg.set_bfd_encapsulation_type(convert_sai_bfd_encapsulation_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_TUNNEL_SRC_IP_ADDRESS:
	msg.set_tunnel_src_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  case SAI_BFD_SESSION_ATTR_TUNNEL_DST_IP_ADDRESS:
	msg.set_tunnel_dst_ip_address(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  ...
  case SAI_BFD_SESSION_ATTR_OFFLOAD_TYPE:
	msg.set_offload_type(convert_sai_bfd_session_offload_type_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_bfd_session(sai_object_id_t *bfd_session_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
	
	lemming::dataplane::sai::CreateBfdSessionRequest req = convert_create_bfd_session(switch_id, attr_count, attr_list);
...
sai_status_t l_set_bfd_session_attribute(sai_object_id_t bfd_session_id, const sai_attribute_t *attr) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
sai_status_t l_get_bfd_session_attribute(sai_object_id_t bfd_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
...
switch (attr_list[i].id) {
  ...
  case SAI_BFD_SESSION_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_bfd_session_type_t_to_sai(resp.attr().type());
	break;
  case SAI_BFD_SESSION_ATTR_HW_LOOKUP_VALID:
  ...
  case SAI_BFD_SESSION_ATTR_VLAN_HEADER_VALID:
	...
	break;
  case SAI_BFD_SESSION_ATTR_BFD_ENCAPSULATION_TYPE:
	 attr_list[i].value.s32 =  convert_sai_bfd_encapsulation_type_t_to_sai(resp.attr().bfd_encapsulation_type());
	break;
  case SAI_BFD_SESSION_ATTR_IPHDR_VERSION:
  ...
  case SAI_BFD_SESSION_ATTR_TUNNEL_TTL:
	...
	break;
  case SAI_BFD_SESSION_ATTR_TUNNEL_SRC_IP_ADDRESS:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().tunnel_src_ip_address());
	break;
  case SAI_BFD_SESSION_ATTR_TUNNEL_DST_IP_ADDRESS:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().tunnel_dst_ip_address());
	break;
  case SAI_BFD_SESSION_ATTR_SRC_MAC_ADDRESS:
  ...
  case SAI_BFD_SESSION_ATTR_STATE:
	 attr_list[i].value.s32 =  convert_sai_bfd_session_state_t_to_sai(resp.attr().state());
	break;
  case SAI_BFD_SESSION_ATTR_OFFLOAD_TYPE:
	 attr_list[i].value.s32 =  convert_sai_bfd_session_offload_type_t_to_sai(resp.attr().offload_type());
	break;
...
sai_status_t l_get_bfd_session_stats(sai_object_id_t bfd_session_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids, uint64_t *counters) {
	...
	for (uint32_t i = 0; i < number_of_counters; i++) {
		req.add_counter_ids(convert_sai_bfd_session_stat_t_to_proto(counter_ids[i]));
	}
...
sai_status_t l_clear_bfd_session_stats(sai_object_id_t bfd_session_id, uint32_t number_of_counters, const sai_stat_id_t *counter_ids) {
	LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
