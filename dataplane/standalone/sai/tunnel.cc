...
const sai_tunnel_api_t l_tunnel = {
	...
	.set_tunnel_term_table_entry_attribute = l_set_tunnel_term_table_entry_attribute,
	.get_tunnel_term_table_entry_attribute = l_get_tunnel_term_table_entry_attribute,
...
lemming::dataplane::sai::CreateTunnelMapRequest convert_create_tunnel_map(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_MAP_ATTR_TYPE:
	msg.set_type(convert_sai_tunnel_map_type_t_to_proto(attr_list[i].value.s32));
	break;
}
...
lemming::dataplane::sai::CreateTunnelRequest convert_create_tunnel(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_ATTR_TYPE:
	msg.set_type(convert_sai_tunnel_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_PEER_MODE:
	msg.set_peer_mode(convert_sai_tunnel_peer_mode_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_SRC_IP:
	msg.set_encap_src_ip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DST_IP:
	msg.set_encap_dst_ip(convert_from_ip_address(attr_list[i].value.ipaddr));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_TTL_MODE:
	msg.set_encap_ttl_mode(convert_sai_tunnel_ttl_mode_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_TTL_VAL:
	msg.set_encap_ttl_val(attr_list[i].value.u8);
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DSCP_MODE:
	msg.set_encap_dscp_mode(convert_sai_tunnel_dscp_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_ENCAP_ECN_MODE:
	msg.set_encap_ecn_mode(convert_sai_tunnel_encap_ecn_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_DECAP_ECN_MODE:
	msg.set_decap_ecn_mode(convert_sai_tunnel_decap_ecn_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_DECAP_TTL_MODE:
	msg.set_decap_ttl_mode(convert_sai_tunnel_ttl_mode_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_TUNNEL_ATTR_DECAP_DSCP_MODE:
	msg.set_decap_dscp_mode(convert_sai_tunnel_dscp_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_VXLAN_UDP_SPORT_MODE:
	msg.set_vxlan_udp_sport_mode(convert_sai_tunnel_vxlan_udp_sport_mode_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_ATTR_IPSEC_SA_PORT_LIST:
	msg.mutable_ipsec_sa_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
...
lemming::dataplane::sai::CreateTunnelTermTableEntryRequest convert_create_tunnel_term_table_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_TYPE:
	msg.set_type(convert_sai_tunnel_term_table_entry_type_t_to_proto(attr_list[i].value.s32));
	break;
...
lemming::dataplane::sai::CreateTunnelMapEntryRequest convert_create_tunnel_map_entry(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_MAP_ENTRY_ATTR_TUNNEL_MAP_TYPE:
	msg.set_tunnel_map_type(convert_sai_tunnel_map_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_TUNNEL_MAP_ENTRY_ATTR_VSID_ID_VALUE:
	...
	break;
  case SAI_TUNNEL_MAP_ENTRY_ATTR_PREFIX_AGG_ID_KEY:
	msg.set_prefix_agg_id_key(attr_list[i].value.u32);
	break;
  case SAI_TUNNEL_MAP_ENTRY_ATTR_SRV6_VPN_SID_VALUE:
	msg.set_srv6_vpn_sid_value(attr_list[i].value.ip6, sizeof(attr_list[i].value.ip6));
	break;
}
...
sai_status_t l_create_tunnel_map(sai_object_id_t *tunnel_map_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTunnelMapRequest req = convert_create_tunnel_map(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_MAP_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_map_type_t_to_sai(resp.attr().type());
	break;
...
switch (attr->id) {
  ...
  case SAI_TUNNEL_ATTR_ENCAP_TTL_MODE:
	req.set_encap_ttl_mode(convert_sai_tunnel_ttl_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_TTL_VAL:
	req.set_encap_ttl_val(attr->value.u8);
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DSCP_MODE:
	req.set_encap_dscp_mode(convert_sai_tunnel_dscp_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DSCP_VAL:
  ...
  case SAI_TUNNEL_ATTR_ENCAP_GRE_KEY:
	...
	break;
  case SAI_TUNNEL_ATTR_DECAP_TTL_MODE:
	req.set_decap_ttl_mode(convert_sai_tunnel_ttl_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_DECAP_DSCP_MODE:
	req.set_decap_dscp_mode(convert_sai_tunnel_dscp_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_LOOPBACK_PACKET_ACTION:
	req.set_loopback_packet_action(convert_sai_packet_action_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_VXLAN_UDP_SPORT_MODE:
	req.set_vxlan_udp_sport_mode(convert_sai_tunnel_vxlan_udp_sport_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_TUNNEL_ATTR_VXLAN_UDP_SPORT:
  ...
  case SAI_TUNNEL_ATTR_SA_INDEX:
	...
	break;
  case SAI_TUNNEL_ATTR_IPSEC_SA_PORT_LIST:
	req.mutable_ipsec_sa_port_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_TUNNEL_ATTR_ENCAP_QOS_TC_AND_COLOR_TO_DSCP_MAP:
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_type_t_to_sai(resp.attr().type());
	break;
  case SAI_TUNNEL_ATTR_UNDERLAY_INTERFACE:
  ...
  case SAI_TUNNEL_ATTR_OVERLAY_INTERFACE:
	...
	break;
  case SAI_TUNNEL_ATTR_PEER_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_peer_mode_t_to_sai(resp.attr().peer_mode());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_SRC_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().encap_src_ip());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DST_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().encap_dst_ip());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_TTL_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_ttl_mode_t_to_sai(resp.attr().encap_ttl_mode());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_TTL_VAL:
	 attr_list[i].value.u8 =   resp.attr().encap_ttl_val();
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DSCP_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_dscp_mode_t_to_sai(resp.attr().encap_dscp_mode());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_DSCP_VAL:
  ...
  case SAI_TUNNEL_ATTR_ENCAP_GRE_KEY:
	...
	break;
  case SAI_TUNNEL_ATTR_ENCAP_ECN_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_encap_ecn_mode_t_to_sai(resp.attr().encap_ecn_mode());
	break;
  case SAI_TUNNEL_ATTR_ENCAP_MAPPERS:
	copy_list(attr_list[i].value.objlist.list, resp.attr().encap_mappers(), &attr_list[i].value.objlist.count);
	break;
  case SAI_TUNNEL_ATTR_DECAP_ECN_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_decap_ecn_mode_t_to_sai(resp.attr().decap_ecn_mode());
	break;
  case SAI_TUNNEL_ATTR_DECAP_MAPPERS:
	copy_list(attr_list[i].value.objlist.list, resp.attr().decap_mappers(), &attr_list[i].value.objlist.count);
	break;
  case SAI_TUNNEL_ATTR_DECAP_TTL_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_ttl_mode_t_to_sai(resp.attr().decap_ttl_mode());
	break;
  case SAI_TUNNEL_ATTR_DECAP_DSCP_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_dscp_mode_t_to_sai(resp.attr().decap_dscp_mode());
	break;
  case SAI_TUNNEL_ATTR_TERM_TABLE_ENTRY_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().term_table_entry_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_TUNNEL_ATTR_LOOPBACK_PACKET_ACTION:
	 attr_list[i].value.s32 =  convert_sai_packet_action_t_to_sai(resp.attr().loopback_packet_action());
	break;
  case SAI_TUNNEL_ATTR_VXLAN_UDP_SPORT_MODE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_vxlan_udp_sport_mode_t_to_sai(resp.attr().vxlan_udp_sport_mode());
	break;
  case SAI_TUNNEL_ATTR_VXLAN_UDP_SPORT:
  ...
  case SAI_TUNNEL_ATTR_SA_INDEX:
	...
	break;
  case SAI_TUNNEL_ATTR_IPSEC_SA_PORT_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().ipsec_sa_port_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_TUNNEL_ATTR_ENCAP_QOS_TC_AND_COLOR_TO_DSCP_MAP:
...
sai_status_t l_create_tunnel_term_table_entry(sai_object_id_t *tunnel_term_table_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTunnelTermTableEntryRequest req = convert_create_tunnel_term_table_entry(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateTunnelTermTableEntryResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = tunnel->CreateTunnelTermTableEntry(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_tunnel_term_table_entry(sai_object_id_t tunnel_term_table_entry_id) {
	...
	grpc::Status status = tunnel->RemoveTunnelTermTableEntry(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr->id) {
  ...
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_PRIORITY:
	...
	grpc::Status status = tunnel->SetTunnelTermTableEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_tunnel_term_table_entry_attribute(sai_object_id_t tunnel_term_table_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_tunnel_term_table_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = tunnel->GetTunnelTermTableEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_VR_ID:
	...
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_term_table_entry_type_t_to_sai(resp.attr().type());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_DST_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().dst_ip());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_DST_IP_MASK:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().dst_ip_mask());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_SRC_IP:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().src_ip());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_SRC_IP_MASK:
	 attr_list[i].value.ipaddr =  convert_to_ip_address(resp.attr().src_ip_mask());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_TUNNEL_TYPE:
  ...
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_IP_ADDR_FAMILY:
	 attr_list[i].value.s32 =  convert_sai_ip_addr_family_t_to_sai(resp.attr().ip_addr_family());
	break;
  case SAI_TUNNEL_TERM_TABLE_ENTRY_ATTR_IPSEC_VERIFIED:
...
sai_status_t l_create_tunnel_map_entry(sai_object_id_t *tunnel_map_entry_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateTunnelMapEntryRequest req = convert_create_tunnel_map_entry(switch_id, attr_count, attr_list);
...
sai_status_t l_get_tunnel_map_entry_attribute(sai_object_id_t tunnel_map_entry_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_tunnel_map_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = tunnel->GetTunnelMapEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_TUNNEL_MAP_ENTRY_ATTR_TUNNEL_MAP_TYPE:
	 attr_list[i].value.s32 =  convert_sai_tunnel_map_type_t_to_sai(resp.attr().tunnel_map_type());
	break;
  case SAI_TUNNEL_MAP_ENTRY_ATTR_TUNNEL_MAP:
  ...
  case SAI_TUNNEL_MAP_ENTRY_ATTR_VSID_ID_VALUE:
	...
	break;
  case SAI_TUNNEL_MAP_ENTRY_ATTR_PREFIX_AGG_ID_KEY:
	 attr_list[i].value.u32 =   resp.attr().prefix_agg_id_key();
	break;
  case SAI_TUNNEL_MAP_ENTRY_ATTR_SRV6_VPN_SID_VALUE:
	memcpy(attr_list[i].value.ip6, resp.attr().srv6_vpn_sid_value().data(), sizeof(sai_ip6_t));
	break;
}
