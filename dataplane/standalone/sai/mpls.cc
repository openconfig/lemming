...
lemming::dataplane::sai::CreateInsegEntryRequest convert_create_inseg_entry(uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_PSC_TYPE:
	msg.set_psc_type(convert_sai_inseg_entry_psc_type_t_to_proto(attr_list[i].value.s32));
	break;
  ...
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	msg.set_pop_ttl_mode(convert_sai_inseg_entry_pop_ttl_mode_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	msg.set_pop_qos_mode(convert_sai_inseg_entry_pop_qos_mode_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_inseg_entry(const sai_inseg_entry_t *inseg_entry, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateInsegEntryRequest req = convert_create_inseg_entry(attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_NEXT_HOP_ID:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_PSC_TYPE:
	req.set_psc_type(convert_sai_inseg_entry_psc_type_t_to_proto(attr->value.s32));
	break;
  case SAI_INSEG_ENTRY_ATTR_QOS_TC:
  ...
  case SAI_INSEG_ENTRY_ATTR_MPLS_EXP_TO_COLOR_MAP:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	req.set_pop_ttl_mode(convert_sai_inseg_entry_pop_ttl_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	req.set_pop_qos_mode(convert_sai_inseg_entry_pop_qos_mode_t_to_proto(attr->value.s32));
	break;
  case SAI_INSEG_ENTRY_ATTR_COUNTER_ID:
...
switch (attr_list[i].id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_NEXT_HOP_ID:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_PSC_TYPE:
	 attr_list[i].value.s32 =  convert_sai_inseg_entry_psc_type_t_to_sai(resp.attr().psc_type());
	break;
  case SAI_INSEG_ENTRY_ATTR_QOS_TC:
  ...
  case SAI_INSEG_ENTRY_ATTR_MPLS_EXP_TO_COLOR_MAP:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	 attr_list[i].value.s32 =  convert_sai_inseg_entry_pop_ttl_mode_t_to_sai(resp.attr().pop_ttl_mode());
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	 attr_list[i].value.s32 =  convert_sai_inseg_entry_pop_qos_mode_t_to_sai(resp.attr().pop_qos_mode());
	break;
  case SAI_INSEG_ENTRY_ATTR_COUNTER_ID:
...
sai_status_t l_remove_inseg_entries(uint32_t object_count, const sai_inseg_entry_t *inseg_entry, sai_bulk_op_error_mode_t mode, sai_status_t *object_statuses) {
	...
	lemming::dataplane::sai::RemoveInsegEntriesRequest req;
	...
	grpc::Status status = mpls->RemoveInsegEntries(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
