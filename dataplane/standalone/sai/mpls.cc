...
switch (attr_list[i].id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_PSC_TYPE:
	req.set_psc_type(static_cast<lemming::dataplane::sai::InsegEntryPscType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	req.set_pop_ttl_mode(static_cast<lemming::dataplane::sai::InsegEntryPopTtlMode>(attr_list[i].value.s32 + 1));
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	req.set_pop_qos_mode(static_cast<lemming::dataplane::sai::InsegEntryPopQosMode>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_NEXT_HOP_ID:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_PSC_TYPE:
	req.set_psc_type(static_cast<lemming::dataplane::sai::InsegEntryPscType>(attr->value.s32 + 1));
	break;
  case SAI_INSEG_ENTRY_ATTR_QOS_TC:
  ...
  case SAI_INSEG_ENTRY_ATTR_MPLS_EXP_TO_COLOR_MAP:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	req.set_pop_ttl_mode(static_cast<lemming::dataplane::sai::InsegEntryPopTtlMode>(attr->value.s32 + 1));
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	req.set_pop_qos_mode(static_cast<lemming::dataplane::sai::InsegEntryPopQosMode>(attr->value.s32 + 1));
	break;
  case SAI_INSEG_ENTRY_ATTR_COUNTER_ID:
...
sai_status_t l_get_inseg_entry_attribute(const sai_inseg_entry_t *inseg_entry, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::InsegEntryAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_INSEG_ENTRY_ATTR_MPLS_EXP_TO_COLOR_MAP:
	...
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_TTL_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().pop_ttl_mode() - 1);
	break;
  case SAI_INSEG_ENTRY_ATTR_POP_QOS_MODE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().pop_qos_mode() - 1);
	break;
  case SAI_INSEG_ENTRY_ATTR_COUNTER_ID:
