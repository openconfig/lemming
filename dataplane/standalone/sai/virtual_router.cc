...
lemming::dataplane::sai::CreateVirtualRouterRequest convert_create_virtual_router(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_TTL1_PACKET_ACTION:
	msg.set_violation_ttl1_packet_action(convert_sai_packet_action_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_IP_OPTIONS_PACKET_ACTION:
	msg.set_violation_ip_options_packet_action(convert_sai_packet_action_t_to_proto(attr_list[i].value.s32));
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_UNKNOWN_L3_MULTICAST_PACKET_ACTION:
	msg.set_unknown_l3_multicast_packet_action(convert_sai_packet_action_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_virtual_router(sai_object_id_t *virtual_router_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateVirtualRouterRequest req = convert_create_virtual_router(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateVirtualRouterResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = virtual_router->CreateVirtualRouter(&context, req, &resp);
	if (!status.ok()) {
...
sai_status_t l_remove_virtual_router(sai_object_id_t virtual_router_id) {
	...
	grpc::Status status = virtual_router->RemoveVirtualRouter(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr->id) {
  ...
  case SAI_VIRTUAL_ROUTER_ATTR_SRC_MAC_ADDRESS:
	...
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_TTL1_PACKET_ACTION:
	req.set_violation_ttl1_packet_action(convert_sai_packet_action_t_to_proto(attr->value.s32));
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_IP_OPTIONS_PACKET_ACTION:
	req.set_violation_ip_options_packet_action(convert_sai_packet_action_t_to_proto(attr->value.s32));
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_UNKNOWN_L3_MULTICAST_PACKET_ACTION:
	req.set_unknown_l3_multicast_packet_action(convert_sai_packet_action_t_to_proto(attr->value.s32));
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_LABEL:
	...
	grpc::Status status = virtual_router->SetVirtualRouterAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_virtual_router_attribute(sai_object_id_t virtual_router_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_virtual_router_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = virtual_router->GetVirtualRouterAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_TTL1_PACKET_ACTION:
	...
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_VIOLATION_IP_OPTIONS_PACKET_ACTION:
	 attr_list[i].value.s32 =  convert_sai_packet_action_t_to_sai(resp.attr().violation_ip_options_packet_action());
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_UNKNOWN_L3_MULTICAST_PACKET_ACTION:
	 attr_list[i].value.s32 =  convert_sai_packet_action_t_to_sai(resp.attr().unknown_l3_multicast_packet_action());
	break;
  case SAI_VIRTUAL_ROUTER_ATTR_LABEL:
