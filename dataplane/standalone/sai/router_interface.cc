...
switch (attr_list[i].id) {
  ...
  case SAI_ROUTER_INTERFACE_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::RouterInterfaceType>(attr_list[i].value.s32 + 1));
	break;
  ...
  case SAI_ROUTER_INTERFACE_ATTR_NEIGHBOR_MISS_PACKET_ACTION:
	req.set_neighbor_miss_packet_action(static_cast<lemming::dataplane::sai::PacketAction>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_ROUTER_INTERFACE_ATTR_EGRESS_ACL:
	...
	break;
  case SAI_ROUTER_INTERFACE_ATTR_NEIGHBOR_MISS_PACKET_ACTION:
	req.set_neighbor_miss_packet_action(static_cast<lemming::dataplane::sai::PacketAction>(attr->value.s32 + 1));
	break;
  case SAI_ROUTER_INTERFACE_ATTR_V4_MCAST_ENABLE:
...
sai_status_t l_get_router_interface_attribute(sai_object_id_t router_interface_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::RouterInterfaceAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_ROUTER_INTERFACE_ATTR_EGRESS_ACL:
	...
	break;
  case SAI_ROUTER_INTERFACE_ATTR_NEIGHBOR_MISS_PACKET_ACTION:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().neighbor_miss_packet_action() - 1);
	break;
  case SAI_ROUTER_INTERFACE_ATTR_V4_MCAST_ENABLE:
