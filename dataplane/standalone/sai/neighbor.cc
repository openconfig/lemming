...
lemming::dataplane::sai::CreateNeighborEntryRequest convert_create_neighbor_entry(uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_neighbor_entry(const sai_neighbor_entry_t *neighbor_entry, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateNeighborEntryRequest req = convert_create_neighbor_entry(attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_NEIGHBOR_ENTRY_ATTR_IS_LOCAL:
	...
	grpc::Status status = neighbor->SetNeighborEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_neighbor_entry_attribute(const sai_neighbor_entry_t *neighbor_entry, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_neighbor_entry_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = neighbor->GetNeighborEntryAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_neighbor_entries(uint32_t object_count, const sai_neighbor_entry_t *neighbor_entry, sai_bulk_op_error_mode_t mode, sai_status_t *object_statuses) {
	...
	for (uint32_t i = 0; i < object_count; i++) {
		 ...
		 *req.add_reqs()->mutable_entry() = convert_from_neighbor_entry(neighbor_entry[i]); 
	}
