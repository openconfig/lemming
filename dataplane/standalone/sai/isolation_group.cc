...
lemming::dataplane::sai::CreateIsolationGroupRequest convert_create_isolation_group(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_ISOLATION_GROUP_ATTR_TYPE:
	msg.set_type(convert_sai_isolation_group_type_t_to_proto(attr_list[i].value.s32));
	break;
}
...
lemming::dataplane::sai::CreateIsolationGroupMemberRequest convert_create_isolation_group_member(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
sai_status_t l_create_isolation_group(sai_object_id_t *isolation_group_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIsolationGroupRequest req = convert_create_isolation_group(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateIsolationGroupResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = isolation_group->CreateIsolationGroup(&context, req, &resp);
	if (!status.ok()) {
...
sai_status_t l_remove_isolation_group(sai_object_id_t isolation_group_id) {
	...
	grpc::Status status = isolation_group->RemoveIsolationGroup(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_isolation_group_attribute(sai_object_id_t isolation_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_isolation_group_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = isolation_group->GetIsolationGroupAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
switch (attr_list[i].id) {
  ...
  case SAI_ISOLATION_GROUP_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_isolation_group_type_t_to_sai(resp.attr().type());
	break;
  case SAI_ISOLATION_GROUP_ATTR_ISOLATION_MEMBER_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().isolation_member_list(), &attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_create_isolation_group_member(sai_object_id_t *isolation_group_member_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateIsolationGroupMemberRequest req = convert_create_isolation_group_member(switch_id, attr_count, attr_list);
	lemming::dataplane::sai::CreateIsolationGroupMemberResponse resp;
	grpc::ClientContext context;
	 req.set_switch_(switch_id); 
	
	grpc::Status status = isolation_group->CreateIsolationGroupMember(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_remove_isolation_group_member(sai_object_id_t isolation_group_member_id) {
	...
	grpc::Status status = isolation_group->RemoveIsolationGroupMember(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_isolation_group_member_attribute(sai_object_id_t isolation_group_member_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_isolation_group_member_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = isolation_group->GetIsolationGroupMemberAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
