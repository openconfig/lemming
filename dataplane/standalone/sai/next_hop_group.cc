...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_GROUP_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::NextHopGroupType>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_next_hop_group_attribute(sai_object_id_t next_hop_group_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NextHopGroupAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_GROUP_MEMBER_ATTR_CONFIGURED_ROLE:
	req.set_configured_role(static_cast<lemming::dataplane::sai::NextHopGroupMemberConfiguredRole>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_next_hop_group_member_attribute(sai_object_id_t next_hop_group_member_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NextHopGroupMemberAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_GROUP_MEMBER_ATTR_WEIGHT:
	...
	break;
  case SAI_NEXT_HOP_GROUP_MEMBER_ATTR_CONFIGURED_ROLE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().configured_role() - 1);
	break;
  case SAI_NEXT_HOP_GROUP_MEMBER_ATTR_OBSERVED_ROLE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().observed_role() - 1);
	break;
  case SAI_NEXT_HOP_GROUP_MEMBER_ATTR_MONITORED_OBJECT:
...
switch (attr_list[i].id) {
  ...
  case SAI_NEXT_HOP_GROUP_MAP_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::NextHopGroupMapType>(attr_list[i].value.s32 + 1));
	break;
}
...
sai_status_t l_get_next_hop_group_map_attribute(sai_object_id_t next_hop_group_map_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::NextHopGroupMapAttr>(attr_list[i].id + 1));
	}
