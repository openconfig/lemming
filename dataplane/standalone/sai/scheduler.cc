...
switch (attr_list[i].id) {
  ...
  case SAI_SCHEDULER_ATTR_SCHEDULING_TYPE:
	req.set_scheduling_type(static_cast<lemming::dataplane::sai::SchedulingType>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_SCHEDULER_ATTR_SCHEDULING_TYPE:
	req.set_scheduling_type(static_cast<lemming::dataplane::sai::SchedulingType>(attr->value.s32 + 1));
	break;
  case SAI_SCHEDULER_ATTR_SCHEDULING_WEIGHT:
	req.set_scheduling_weight(attr->value.u8);
	break;
  case SAI_SCHEDULER_ATTR_METER_TYPE:
	req.set_meter_type(static_cast<lemming::dataplane::sai::MeterType>(attr->value.s32 + 1));
	break;
  case SAI_SCHEDULER_ATTR_MIN_BANDWIDTH_RATE:
...
sai_status_t l_get_scheduler_attribute(sai_object_id_t scheduler_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::SchedulerAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_SCHEDULER_ATTR_SCHEDULING_TYPE:
	 attr_list[i].value.s32 =  static_cast<int>(resp.attr().scheduling_type() - 1);
	break;
  case SAI_SCHEDULER_ATTR_SCHEDULING_WEIGHT:
