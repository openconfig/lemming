...
lemming::dataplane::sai::CreateDtelRequest convert_create_dtel(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_ATTR_SINK_PORT_LIST:
	msg.mutable_sink_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
  case SAI_DTEL_ATTR_INT_L4_DSCP:
	*msg.mutable_int_l4_dscp() = convert_from_acl_field_data( attr_list[i].value.aclfield, attr_list[i].value.aclfield.data.u8, attr_list[i].value.aclfield.mask.u8 );
	break;
}
...
lemming::dataplane::sai::CreateDtelQueueReportRequest convert_create_dtel_queue_report(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateDtelIntSessionRequest convert_create_dtel_int_session(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateDtelReportSessionRequest convert_create_dtel_report_session(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
lemming::dataplane::sai::CreateDtelEventRequest convert_create_dtel_event(sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_EVENT_ATTR_TYPE:
	msg.set_type(convert_sai_dtel_event_type_t_to_proto(attr_list[i].value.s32));
	break;
...
sai_status_t l_create_dtel(sai_object_id_t *dtel_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDtelRequest req = convert_create_dtel(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_DTEL_ATTR_LATENCY_SENSITIVITY:
	...
	break;
  case SAI_DTEL_ATTR_SINK_PORT_LIST:
	req.mutable_sink_port_list()->Add(attr->value.objlist.list, attr->value.objlist.list + attr->value.objlist.count);
	break;
  case SAI_DTEL_ATTR_INT_L4_DSCP:
	*req.mutable_int_l4_dscp() = convert_from_acl_field_data( attr->value.aclfield, attr->value.aclfield.data.u8, attr->value.aclfield.mask.u8 );
	break;
}
...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_ATTR_LATENCY_SENSITIVITY:
	...
	break;
  case SAI_DTEL_ATTR_SINK_PORT_LIST:
	copy_list(attr_list[i].value.objlist.list, resp.attr().sink_port_list(), &attr_list[i].value.objlist.count);
	break;
  case SAI_DTEL_ATTR_INT_L4_DSCP:
	 attr_list[i].value.aclfield =  convert_to_acl_field_data_u8(resp.attr().int_l4_dscp(), resp.attr().int_l4_dscp().data_uint(), resp.attr().int_l4_dscp().mask_uint());
	break;
}
...
sai_status_t l_create_dtel_queue_report(sai_object_id_t *dtel_queue_report_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDtelQueueReportRequest req = convert_create_dtel_queue_report(switch_id, attr_count, attr_list);
...
sai_status_t l_get_dtel_queue_report_attribute(sai_object_id_t dtel_queue_report_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_dtel_queue_report_attr_t_to_proto(attr_list[i].id));
	}
...
sai_status_t l_create_dtel_int_session(sai_object_id_t *dtel_int_session_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDtelIntSessionRequest req = convert_create_dtel_int_session(switch_id, attr_count, attr_list);
...
sai_status_t l_get_dtel_int_session_attribute(sai_object_id_t dtel_int_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_dtel_int_session_attr_t_to_proto(attr_list[i].id));
	}
...
sai_status_t l_create_dtel_report_session(sai_object_id_t *dtel_report_session_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDtelReportSessionRequest req = convert_create_dtel_report_session(switch_id, attr_count, attr_list);
...
switch (attr->id) {
  ...
  case SAI_DTEL_REPORT_SESSION_ATTR_UDP_DST_PORT:
	...
	grpc::Status status = dtel->SetDtelReportSessionAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_get_dtel_report_session_attribute(sai_object_id_t dtel_report_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(convert_sai_dtel_report_session_attr_t_to_proto(attr_list[i].id));
	}
	grpc::Status status = dtel->GetDtelReportSessionAttribute(&context, req, &resp);
	if (!status.ok()) {
		auto it = context.GetServerTrailingMetadata().find("traceparent");
		if (it != context.GetServerTrailingMetadata().end()) {
			LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second << " msg: " << status.error_message(); 
...
sai_status_t l_create_dtel_event(sai_object_id_t *dtel_event_id, sai_object_id_t switch_id, uint32_t attr_count, const sai_attribute_t *attr_list) {
	...
	lemming::dataplane::sai::CreateDtelEventRequest req = convert_create_dtel_event(switch_id, attr_count, attr_list);
...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_EVENT_ATTR_TYPE:
	 attr_list[i].value.s32 =  convert_sai_dtel_event_type_t_to_sai(resp.attr().type());
	break;
  case SAI_DTEL_EVENT_ATTR_REPORT_SESSION:
