...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_ATTR_SINK_PORT_LIST:
	req.mutable_sink_port_list()->Add(attr_list[i].value.objlist.list, attr_list[i].value.objlist.list + attr_list[i].value.objlist.count);
	break;
}
...
sai_status_t l_get_dtel_attribute(sai_object_id_t dtel_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::DtelAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_dtel_queue_report_attribute(sai_object_id_t dtel_queue_report_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::DtelQueueReportAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_dtel_int_session_attribute(sai_object_id_t dtel_int_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::DtelIntSessionAttr>(attr_list[i].id + 1));
	}
...
sai_status_t l_get_dtel_report_session_attribute(sai_object_id_t dtel_report_session_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::DtelReportSessionAttr>(attr_list[i].id + 1));
	}
...
switch (attr_list[i].id) {
  ...
  case SAI_DTEL_EVENT_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::DtelEventType>(attr_list[i].value.s32 + 1));
	break;
...
sai_status_t l_get_dtel_event_attribute(sai_object_id_t dtel_event_id, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::DtelEventAttr>(attr_list[i].id + 1));
	}
