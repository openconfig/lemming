...
lemming::dataplane::sai::FlushFdbEntriesRequest convert_flush_fdb_entries(
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    switch (attr_list[i].id) {
      ...
      case SAI_FDB_FLUSH_ATTR_ENTRY_TYPE:
        msg.set_entry_type(convert_sai_fdb_flush_entry_type_t_to_proto(
...
sai_status_t l_flush_fdb_entries(sai_object_id_t switch_id, uint32_t attr_count,
                                 const sai_attribute_t* attr_list) {
  LOG(INFO) << "Func: " << __PRETTY_FUNCTION__;
  lemming::dataplane::sai::FlushFdbEntriesRequest req =
      convert_flush_fdb_entries(switch_id, attr_count, attr_list);
