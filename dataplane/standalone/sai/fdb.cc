...
switch (attr_list[i].id) {
  ...
  case SAI_FDB_ENTRY_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::FdbEntryType>(attr_list[i].value.s32 + 1));
	break;
...
switch (attr->id) {
  ...
  case SAI_FDB_ENTRY_ATTR_TYPE:
	req.set_type(static_cast<lemming::dataplane::sai::FdbEntryType>(attr->value.s32 + 1));
	break;
  case SAI_FDB_ENTRY_ATTR_PACKET_ACTION:
...
sai_status_t l_get_fdb_entry_attribute(const sai_fdb_entry_t *fdb_entry, uint32_t attr_count, sai_attribute_t *attr_list) {
	...
	for (uint32_t i = 0; i < attr_count; i++) {
		req.add_attr_type(static_cast<lemming::dataplane::sai::FdbEntryAttr>(attr_list[i].id + 1));
	}
