...
grpc::Status L2mcGroup::RemoveL2mcGroup(
  ...
  auto status = api->remove_l2mc_group(req.get_oid());

  auto status = api->remove_l2mc_group(entry);
  if (!status.ok()) {
...
grpc::Status L2mcGroup::RemoveL2mcGroupMember(
  ...
  grpc::ClientContext context;
  auto status = api->remove_l2mc_group_member(req.get_oid());

  auto status = api->remove_l2mc_group_member(entry);
  if (!status.ok()) {
    auto it = context.GetServerTrailingMetadata().find("traceparent");
    if (it != context.GetServerTrailingMetadata().end()) {
      LOG(ERROR) << "Lucius RPC error: Trace ID " << it->second
