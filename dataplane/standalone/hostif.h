...
#ifndef DATAPLANE_STANDALONE_HOSTIF_H_
...
class HostIf : public APIBase {
 public:
  HostIf(std::string id, std::shared_ptr<AttributeManager> mgr,
...
class HostIfTableEntry : public APIBase {
 public:
  HostIfTableEntry(std::string id, std::shared_ptr<AttributeManager> mgr,
                   std::shared_ptr<forwarding::Forwarding::Stub> fwd,
                   std::shared_ptr<lemming::dataplane::Dataplane::Stub> dplane)
      : APIBase(id, mgr, fwd, dplane) {}
  ~HostIfTableEntry() = default;
  sai_status_t create(_In_ uint32_t attr_count,
                      _In_ const sai_attribute_t* attr_list);
  sai_status_t set_attribute(_In_ const sai_attribute_t* attr);
};

class HostIfTrap : public APIBase {
 public:
  HostIfTrap(std::string id, std::shared_ptr<AttributeManager> mgr,
             std::shared_ptr<forwarding::Forwarding::Stub> fwd,
             std::shared_ptr<lemming::dataplane::Dataplane::Stub> dplane)
      : APIBase(id, mgr, fwd, dplane) {}
  ~HostIfTrap() = default;
