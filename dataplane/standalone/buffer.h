...
#ifndef DATAPLANE_STANDALONE_BUFFER_H_
...
class BufferPool : public APIBase {
 public:
  BufferPool(std::string id, std::shared_ptr<AttributeManager> mgr,
...
class BufferProfile : public APIBase {
 public:
  BufferProfile(std::string id, std::shared_ptr<AttributeManager> mgr,
                std::shared_ptr<forwarding::Forwarding::Stub> fwd,
                std::shared_ptr<lemming::dataplane::Dataplane::Stub> dplane)
      : APIBase(id, mgr, fwd, dplane) {}
...
#endif  // DATAPLANE_STANDALONE_BUFFER_H_
