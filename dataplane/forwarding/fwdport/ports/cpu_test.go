...
type RecordPacketSink struct {
	notify     chan bool
	lastPacket *fwdpb.PacketInjectRequest
}

// PacketSink records the inject packet request and generates a notification.
func (p *RecordPacketSink) PacketSink(request *fwdpb.PacketInjectRequest) (*fwdpb.PacketInjectReply, error) {
	p.lastPacket = request
	p.notify <- true
...
func TestCpuWrite(t *testing.T) {
	...
	arp := []byte{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
