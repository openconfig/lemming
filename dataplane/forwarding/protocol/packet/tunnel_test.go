...
func TestVXLANPacketParsing(t *testing.T) {
	rawBytes := append([]byte(nil), ethernetIP4...)
	rawBytes = append(rawBytes, headers[outerIP4UDP].orig...)
	// UDP header with dport 4789 (0x12b5)
	rawBytes = append(rawBytes, []byte{0x01, 0x02, 0x12, 0xb5, 0x00, 0x2a, 0x00, 0x00}...)
	// VXLAN header with VNI 20001 (0x004e21)
	rawBytes = append(rawBytes, []byte{0x08, 0x00, 0x00, 0x00, 0x00, 0x4e, 0x21, 0x00}...)
	rawBytes = append(rawBytes, headers[innerEthernet].orig...)
	rawBytes = append(rawBytes, headers[innerIP4].orig...)
	...
	tests := []struct {
		...
		},
		{
			...
			want:    []byte{0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b},
		},
		{
