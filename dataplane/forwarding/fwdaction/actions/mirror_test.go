...
func TestMirror(t *testing.T) {
	...
	for idx, test := range tests {
		...
		mirrored.EXPECT().Field(fwdpacket.NewFieldIDFromNum(fwdpb.PacketFieldNum_PACKET_FIELD_NUM_ETHER_TYPE, 0)).Return([]byte{0x08, 0x00}, nil).AnyTimes()
		...
		original.EXPECT().Field(fwdpacket.NewFieldIDFromNum(fwdpb.PacketFieldNum_PACKET_FIELD_NUM_ETHER_TYPE, 0)).Return([]byte{0x08, 0x00}, nil).AnyTimes()
