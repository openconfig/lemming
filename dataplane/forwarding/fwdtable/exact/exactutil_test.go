...
// TestPacketField verifies that makeEntryKey accepts and rejects packet
...
func TestPacketFields(t *testing.T) {
	...
	for pos, test := range tests {
		...
		_, err := newExactKey(kd, ed)
		if str := ErrTest(err, test.err); str != "" {
			t.Errorf("%d: newExactKey failed, err %v.", pos, str)
		}
	}
}
...
// TestPacketBytes verifies that makeEntryKey accepts and rejects packet
// bytes appropriately.
